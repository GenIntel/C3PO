...
class OD3D_Results(Dict[str, Union[torch.Tensor, List]]):
    def __init__(
        ...
        init_dict: Dict[str, Union[torch.Tensor, List]] = None,
        logging_dir=None,
    ):
        super().__init__()
        self.mean_blocklist = [
        ...
        for key, val in other.items():
            ...
            if isinstance(val, torch.Tensor):
                ...
                if key not in self.keys():
                    self[key] = val.to(
                        device=self.device,
                    )  # torch.Tensor(size=val.shape, device=self.device, dtype=val.dtype)
                else:
                    self[key] = torch.cat(
                        [self[key], val.to(device=self.device)],
                        dim=0,
                    )
    ...
    def mean(self):
        ...
        for key, val in self.items():
            if not any(
                s in key for s in self.mean_blocklist
            ):  #  key not in self.mean_blocklist:
        ...
        for k in self.keys():
                    ...
                    if f"{prefix}label_names" in self.keys():
                        ...
                        label_names = [
                            ...
                            for i in range(
                                max(
                                    set(
                                        self[f"{prefix}label_gt"]
                                        + self[f"{prefix}label_pred"],
                                    ),
                                )
                                + 1,
                            )
                        ]
                    res[f"label/{prefix_saved}acc"] = (
                        (self[f"{prefix}label_gt"] == self[f"{prefix}label_pred"])
                        .to(dtype=float)
                        .mean(dim=0)
                    )
                    res[f"label/{prefix_saved}confusion"] = wandb.plot.confusion_matrix(
                        probs=None,
            ...
            if "rot_diff_rad" in k:
                ...
                res[f"pose/{prefix_saved}acc_pi6"] = (
                    (rot_diff_rad < math.pi / 6.0).to(dtype=float).mean()
                )
                res[f"pose/{prefix_saved}acc_pi12"] = (
                    (rot_diff_rad < math.pi / 12.0).to(dtype=float).mean()
                )
                ...
                if (
                    ...
                    and f"{prefix}pose_sim_appear" in self.keys()
                ):
                    res[
                        f"pose/pr/{prefix_saved}pi6_pr_vs_sim_geo_and_appear"
                    ] = self.get_pr_3d(
                        ground_truth=(rot_diff_rad.flatten() < math.pi / 6.0)
                        .detach()
                        .cpu()
                        .numpy()
                        .astype(int),
                        sim_1st_dim=self[f"{prefix}pose_sim_geo"]
                        .flatten()
                        .detach()
                        .cpu()
                        .numpy(),
                        sim_2nd_dim=self[f"{prefix}pose_sim_appear"]
                        .flatten()
                        .detach()
                        .cpu()
                        .numpy(),
                ...
                if f"{prefix}sim" in self.keys():
                    res[f"pose/pr/{prefix_saved}pi6"] = self.get_pr(
                        ground_truth=(rot_diff_rad.flatten() < math.pi / 6.0)
                        .detach()
                        .cpu()
                        .numpy()
                        .astype(int),
                        predictions=self[f"{prefix}sim"]
                        .flatten()
                        .detach()
                        .cpu()
                        .numpy(),
                    ...
                    res[f"pose/pr/{prefix_saved}pi18"] = self.get_pr(
                        ground_truth=(rot_diff_rad.flatten() < math.pi / 18.0)
                        .detach()
                        .cpu()
                        .numpy()
                        .astype(int),
                        ...
                        .cpu()
                        .numpy(),
                        title=f"PI/18={res[f'pose/{prefix_saved}acc_pi18']:.2f}",
                    )
    ...
    def get_pr_3d(self, ground_truth, sim_1st_dim, sim_2nd_dim, title):
        ...
        mask_xy = (sim_1st_dim[:, None, None] >= X[None,]) * (
            sim_2nd_dim[:, None, None] >= Y[None,]
        )
        Z_precision = (ground_truth[:, None, None] * mask_xy).sum(axis=0) / (
            mask_xy.sum(axis=0)
        )
        Z_precision[mask_xy.sum(axis=0) == 0] = 1.0

        Z_recall = (ground_truth[:, None, None] * mask_xy).sum(axis=0) / (
            ground_truth.sum()
        )
        ...
        Z_f1[Z_recall + Z_precision == 0] = 0.0
        ...
        for x_step in range(0, math.ceil(len(X) / stepsize)):
            for y_step in range(0, math.ceil(len(Y) / stepsize)):
                Z_f1_step = Z_f1[
                    (x_step) * stepsize : (x_step + 1) * stepsize,
                    (y_step) * stepsize : (y_step + 1) * stepsize,
                ]
                ...
                text_sub.append(
                    f"Precision={Z_precision[x_amax, y_amax]:.2f}<br>Recall={Z_recall[x_amax, y_amax]:.2f}",
                )
    ...
    def get_filtered_log_results(self):
        ...
        for key, val in self.items():
            if not any(
                s in key for s in self.log_blocklist
            ):  #  key not in self.log_blocklist: # any(substring in s for s in string_list)
                res[key] = val
        return res
    ...
    def log_with_prefix(self, prefix: str, prefix_append_char="/"):
        filtered_log_results = self.get_filtered_log_results()
        filtered_log_results_with_prefix = {
            prefix + f"{prefix_append_char}" + k: v
            for k, v in filtered_log_results.items()
        }
    ...
    def save_with_dataset(self, prefix: str, dataset, _dict: dict = None):
        ...
        dataset.save_to_config(
            fpath=self.logging_dir.joinpath(f"{prefix}/{dataset.name}/config.yaml"),
        )
