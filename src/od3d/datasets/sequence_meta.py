...
class OD3D_SequenceMetaCategoryMixin(OD3D_SequenceMeta):
    ...
    @staticmethod
    def load_from_meta_with_category_and_name(
        path_meta: Path,
        category: str,
        name: str,
    ):
        name_unique = f"{category}/{name}"
        return OD3D_SequenceMetaCategoryMixin.load_from_meta_with_name_unique(
            path_meta=path_meta,
    ...
    @staticmethod
    def get_fpath_sequence_meta_with_category_and_name(
        path_meta: Path,
        category: str,
        name: str,
    ):
        return path_meta.joinpath(
            OD3D_SequenceMetaCategoryMixin.get_rfpath_metas(),
            OD3D_SequenceMetaCategoryMixin.get_name_unique_with_category_and_name(
                category=category,
                name=name,
            ),
        )
