...
from od3d.datasets.dataset import OD3D_FrameMeta, OD3D_Frame
import scipy.io
import math
import numpy as np
...
class Pascal3DFrame(OD3D_Frame):
    ...
    def preprocess_mask(self, override=False):
        if not self.fpath_mask.exists() or override:
            if torch.cuda.is_available():
                ...
                device = 'cpu'
            meshes = Meshes.load_from_files([self.path_meshes.joinpath(self.meta.rfpath_mesh)], device=device)
    ...
    def mesh(self):
        if self._mesh is None:
            self._mesh = Mesh.load_from_file(fpath=self.path_meshes.joinpath(self.meta.rfpath_mesh))
        return self._mesh
