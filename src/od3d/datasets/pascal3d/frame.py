...
class Pascal3DFrameMeta(
    ...
    def load_category_mesh_bbox_kpts2d_cam_from_object_annotation_raw(
        ...
        kpts2d_annot = np.stack(
            [
                object["anchors"][0][0][n]["location"][0][0][0]
                if object["anchors"][0][0][n]["status"] == 1
                else np.array(
                    [0, 0],
                )
                for n in kpts_names
        ...
        kpts2d_annot_vsbl = np.array(
            [
                True if object["anchors"][0][0][n]["status"] == 1 else False
                for n in kpts_names
            ],
        )
    ...
    def load_kpts3d_from_raw(path_raw, rpath_meshes, category, mesh_index, kpts_names):
        ...
        kpts3d = np.stack(
            [
                annotation_mesh3d[category][n][0][mesh_index][0]
                if len(
                    annotation_mesh3d[category][n][0][mesh_index],
                )
                > 0
    ...
    def load_from_raw_annotation(
        ...
        H = int(
            annotation["record"][0][0]["size"]["height"][0][0][0][0],
        )  # self.rgb.shape[1:]
        ...
        if cam_intr4x4[0, 0] == 0.0 or cam_intr4x4[1, 1] == 0.0:
    ...
    def load_from_raw(
        ...
        return Pascal3DFrameMeta.load_from_raw_annotation(
...
class Pascal3DFrame(
    ...
    def read_kpts3d(self):
        kpts3d = (
            self.meta.kpts3d.clone() * PASCAL3D_SCALE_NORMALIZE_TO_REAL[self.category]
        )
        return kpts3d
