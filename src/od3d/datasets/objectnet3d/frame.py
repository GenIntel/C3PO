...
class ObjectNet3D_FrameMeta(
    ...
    def load_from_raw(
        ...
        try:
            ...
            for i in range(objects.shape[0]):
                ...
                objs_bbox.append(
                    torch.from_numpy((objects[i]["bbox"][0]).astype("int")),
                )

                cam_tform4x4_obj = Pascal3DFrame.calc_cam_tform_obj(
                    azimuth=azimuth,
                    elevation=elevation,
                    theta=theta,
                    distance=distance,
                ...
                cam_intr3x3 = np.array(
                    [
                        [1.0 * viewport * focal, 0, px],
                        [0, 1.0 * viewport * focal, py],
                        [0, 0, 1.0],
                    ],
                )
                cam_intr4x4 = np.hstack((cam_intr3x3, [[0], [0], [0]]))
                cam_intr4x4 = np.vstack((cam_intr4x4, [0, 0, 0, 1]))
                cam_intr4x4 = torch.from_numpy(cam_intr4x4).to(
                    dtype=cam_tform4x4_obj.dtype,
                )
                objs_cam_intr4x4.append(cam_intr4x4)

                objs_rfpaths_meshes.append(
                    rfpath_meshes.joinpath(category, f"{(cad_index + 1):02d}.off"),
                )
            ...
            if objs_count == 0:
                ...
                if not ((objs_cam_intr4x4 - objs_cam_intr4x4[:1]) == 0.0).all():
        ...
        return ObjectNet3D_FrameMeta(
            subset=subset,
            name=name,
            rfpath_rgb=rfpath_rgb,
            rfpaths_meshs=objs_rfpaths_meshes,
            l_bboxs=objs_bbox.tolist(),
            l_size=size.tolist(),
            l_cam_tform4x4_objs=objs_cam_tform4x4_obj.tolist(),
    ...
    def get_path_frames_meta_with_subset(path_meta: Path, subset: str):
        return ObjectNet3D_FrameMeta.get_path_metas(path_meta=path_meta).joinpath(
            subset,
        )
    ...
    def read_mesh(self, mesh_type=None):
        ...
        if mesh_type == OD3D_MESH_TYPES.META:
            mesh = Mesh.load_from_file(
                ...
                scale=OBJECTNET3D_SCALE_NORMALIZE_TO_REAL[self.category],
            )
        else:
