...
from od3d.datasets.objectnet3d.enum import (
    OBJECTNET3D_CATEGORIES,
...
from od3d.datasets.objectnet3d.frame import (
    ObjectNet3D_FrameMeta,
    ObjectNet3D_Frame,
)  # , OD3D_Frame
...
class ObjectNet3D(OD3D_Dataset):
    ...
    def filter_list_frames_unique(self, list_frames_unique):
        ...
        if self.filter_frames_categorical:
            ...
            allowed_subsets = {
                f_unique.split("/")[0] for f_unique in list_frames_unique
            }
            for subset in allowed_subsets:
                for category in self.categories:
                    logger.info(f"{subset}, {category}")
                    allowed_frames_unique += self.get_subset_category_names_unique(
                        subset=subset,
                        category=category,
                    )
            list_frames_unique_filtered = list(
                set.intersection(set(allowed_frames_unique), set(list_frames_unique)),
            )
    ...
    def setup(config: DictConfig):
        ...
        for name, url in dict_name_to_url.items():
            ...
            if path_dir.exists() and not config.setup.override:
                ...
                od3d.io.unzip(
                    path_raw.joinpath(f"{name}.zip"),
                    dst=path_raw.joinpath("tmp"),
                )
                od3d.io.move_dir(
                    src=path_raw.joinpath("tmp", "ObjectNet3D"),
                    dst=path_raw,
                )
    ...
    def extract_meta(config: DictConfig):
        ...
        for subset in subsets:
            path_frames_subset = ObjectNet3D_FrameMeta.get_path_frames_meta_with_subset(
                path_meta=path_meta,
                subset=subset,
            )
            if not path_frames_subset.exists() or config.extract_meta.override:
                fpath_image_set_subset = path_raw.joinpath(
                    "Image_sets",
                    subset + ".txt",
                )
                ...
                for i in tqdm(range(len(frames_names))):
                    ...
                    frame_meta = ObjectNet3D_FrameMeta.load_from_raw(
                        path_raw=path_raw,
                        rfpath_annotations=rfpath_annotations,
                        rfpath_images=rfpath_images,
                        rfpath_meshes=rfpath_meshes,
                        subset=subset,
                        name=frame_name,
                    )
    ...
    def preprocess(self, config_preprocess: DictConfig):
        ...
        for key in config_preprocess.keys():
            ...
            if (
                key == "subset_category_names_unique"
                and config_preprocess.subset_category_names_unique.get("enabled", False)
            ):
                override = config_preprocess.subset_category_names_unique.get(
                    "override",
                    False,
                )
                remove_previous = config_preprocess.subset_category_names_unique.get(
                    "remove_previous",
                    False,
                )
                self.preprocess_subset_category_names_unique(
                    override=override,
                    remove_previous=remove_previous,
                )
        ...
        for category in self.categories:
            ...
            for mesh_type in mesh_types:
                fpath_mesh_out = self.path_preprocess.joinpath(
                    ObjectNet3D_Frame.get_rfpath_pp_categorical_mesh(
                        mesh_type=mesh_type,
                        category=category,
                    ),
                )
                ...
                fpaths_meshes_category = [
                    fpath
                    for fpath in self.path_raw.joinpath(
                        ObjectNet3D_Frame.get_rpath_raw_categorical_meshes(
                            category=category,
                        ),
                    ).iterdir()
                ]
                fpaths_meshes_category = [
                    fpath
                    for fpath in fpaths_meshes_category
                    if re.match(r"[0-9][0-9]\.off", fpath.name)
                ]
                ...
                cuboids, tform_obj = fit_cuboid_to_pts3d(
                    ...
                    size=OD3D_CATEGORIES_SIZES_IN_M[
                        MAP_CATEGORIES_OBJECTNET3D_TO_OD3D[category]
                    ],
                )

                scale_objectnet3d_to_od3d[category] = (
    ...
    def preprocess_subset_category_names_unique(
        self,
        override=False,
        ...
        for frame_id in tqdm(range(len(dataset))):
            ...
            if (
                frame_meta.category
                not in dict_subset_category_names_unique[frame_meta.subset]
            ):
                dict_subset_category_names_unique[frame_meta.subset][
                    frame_meta.category
                ] = []
        ...
        for subset in dict_subset_category_names_unique.keys():
            for category in dict_subset_category_names_unique[subset]:
                fpath = self.path_preprocess.joinpath(
                    ...
                    f"{subset}_{category}.yaml",
                )
                if not fpath.exists() or override:
                    od3d.io.write_list_as_yaml(
                        fpath=fpath,
                        _list=dict_subset_category_names_unique[subset][category],
                    )
                else:
                    logger.warning(
                        f"not overriding {fpath}, set override flag if desired.",
                    )
