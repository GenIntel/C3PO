...
class CO3D(OD3D_SequenceDataset):
    ...
    def __init__(
        ...
        dict_nested_frames_annotated: Dict[str, Dict[str, List[str]]] = None,
        dict_nested_frames_ratio_50_mixture: Dict[str, Dict[str, List[str]]] = None,
        dict_nested_frames_ratio_100_mixture: Dict[str, Dict[str, List[str]]] = None,
        ...
        super().__init__(
            ...
            # dict_nested_frames_annotated=dict_nested_frames_annotated,
        ...
        self.use_flipped_feature = use_flipped_feature
        self.use_sd = use_sd
    ...
    def setup(config: DictConfig):
        ...
        if path_raw.exists() and not config.setup.override:
            ...
            run_cmd(
                cmd=f"cd {path_raw} && git clone git@github.com:facebookresearch/co3d.git",
                live=True,
                logger=logger,
            )
            logger.info(f"Downloading CO3D dataset at {path_raw}")
            run_cmd(
                cmd=f'python {path_co3d_repo.joinpath("co3d/download_dataset.py")} --download_folder {path_raw}',
                live=True,
                logger=logger,
            )
    ...
    def extract_meta(config: DictConfig):
        ...
        preprocess_meta_remove_previous = config.get("extract_meta", False).get(
            "remove_previous",
            False,
        )
        ...
        for category in categories:
            ...
            sequence_annotations = load_dataclass_jgzip(
                f"{path}/{category}/sequence_annotations.jgz",
            ...
            if sequences_names is None and (
                ...
                sequences_names = [
                    sequence_annoation.sequence_name
            ...
            for sequence_annoation in tqdm(sequence_annotations):
                if (
                    sequences_names is not None
                    and sequence_annoation.sequence_name not in sequences_names
                ):
                    continue
                ...
                sequence_meta_fpath = (
                    CO3D_SequenceMeta.get_fpath_sequence_meta_with_category_and_name(
                        path_meta=path_meta,
                        category=category,
                        name=sequence_name,
                    )
                ...
                sequence_meta = CO3D_SequenceMeta.load_from_raw(
                    sequence_annotation=sequence_annoation,
                )
            ...
            cls_frame_annotations = load_dataclass_jgzip(
                f"{path}/{category}/frame_annotations.jgz",
                List[FrameAnnotation],
            )
            cls_frame_annotations = [
                fa
                for fa in cls_frame_annotations
                if fa.meta["frame_type"] in ALLOW_LIST_FRAME_TYPES
            ]
            ...
            for frame_annotation in tqdm(cls_frame_annotations):
                if (
                    sequences_names is not None
                    and frame_annotation.sequence_name not in sequences_names
                ):
                    continue
                ...
                frame_meta_fpath = CO3D_FrameMeta.get_fpath_frame_meta_with_category_sequence_and_frame_name(
                    path_meta=path_meta,
                    category=category,
                ...
                frame_meta = CO3D_FrameMeta.load_from_raw(
