...
class Configuration:
    ...
    def load_yaml(self, file):
        with open(file) as stream:
            ...
            for include in includes:
                include = od3d.utils.get_abs_path(include)
    ...
    def _decode_value(self, value):
        ...
        if value.endswith(".yaml"):
            return self.load_yaml(od3d.utils.get_abs_path(value))
...
def load_config(args, load_default_config=True, override=None, log_info=True):
    config_path = od3d.utils.get_abs_path(args.config)
    ...
    if load_default_config:
        config = Configuration(
            config_path,
            default_config=os.path.join(od3d.utils.get_project_root(), "config/defaults.yaml"),
        )
    else:
    ...
    if od3d.utils.is_main_process() and log_info:
        print_str = (
            "\n" + "=" * 16 + "\nConfigurations\n" + "=" * 16 + "\n" + str(config)
        )
