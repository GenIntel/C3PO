...
class NeMo_MultiView(NeMo):
    ...
    def inference_batch_single_view(self, batch, return_samples_with_sim=True):
        if self.config.multiview.type == MultiViewType.MULTIVIEW:
            return self.inference_batch_multiview(
                batch,
                return_samples_with_sim=return_samples_with_sim,
            )
        elif self.config.multiview.type == MultiViewType.SINGLE:
            return super().inference_batch_single_view(
                batch,
                return_samples_with_sim=return_samples_with_sim,
            )
        elif self.config.multiview.type == MultiViewType.MULTIVIEW_MEAN_SE3:
            results_batch = super().inference_batch_single_view(
                batch,
                return_samples_with_sim=return_samples_with_sim,
            )
            obj_tform4x4_cuboid_front = tform4x4_broadcast(
                inv_tform4x4(batch.cam_tform4x4_obj),
                results_batch["cam_tform4x4_obj"],
            )
            obj_tform6_cuboid_front = se3_log_map(obj_tform4x4_cuboid_front).mean(
                dim=0,
                keepdim=True,
            )
            ...
            diff_rot3x3 = rot3x3(
                batch.cam_tform4x4_obj[:, :3, :3].permute(0, 2, 1),
                cam_tform4x4_obj[:, :3, :3],
            )
            try:
                diff_so3_log = pytorch3d.transforms.so3_log_map(
                    diff_rot3x3.permute(0, 2, 1),
                )
        ...
        with torch.no_grad():
            ...
            for mesh_id in range(len(self.meshes)):
                ...
                inner_feats2d_net_bank_vts_max_vals = (
                    torch.einsum("bchw,kc->bkhw", feats2d_net, bank_feats)
                    .max(
                        dim=1,
                        keepdim=True,
                    )
            ...
            cams_multiview_tform4x4_obj, cams_multiview_intr4x4 = self.get_samples(
            ...
            objs_multiview_tform4x4_cuboid_front = tform4x4_broadcast(
                ...
                cams_multiview_tform4x4_obj,
            )
            b_cams_multiview_tform4x4_obj = tform4x4_broadcast(
                batch.cam_tform4x4_obj[:, None],
                objs_multiview_tform4x4_cuboid_front,
            )
            b_cams_multiview_intr4x4 = cams_multiview_intr4x4.expand(
