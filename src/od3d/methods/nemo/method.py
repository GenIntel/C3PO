...
class NeMo(OD3DMethod):
    ...
    def train(self, dataset: OD3D_Dataset):
        ...
        dataloader_train = torch.utils.data.DataLoader(dataset=dataset_train, batch_size=2, shuffle=False,
        ...
        for e in range(self.config.training.total_epochs):
            self.test(dataset_val, complete_dataset=True, pose_iterative_refine=False)
