...
class NeMo(OD3DMethod):
    ...
    def load_checkpoint_old(self, path_checkpoint):
        ...
        for i in range(len(self.fpaths_meshes)):
            ...
            verts_feats.append(checkpoint["memory"][mesh_old_id * verts_count_max: (
                                                                                               mesh_old_id + 1) * verts_count_max].clone().detach().cpu())
    ...
    def train(self, dataset: OD3D_Dataset, datasets_val: List[OD3D_Dataset]):
        ...
        for epoch in range(self.config.train.epochs):
            ...
            if self.config.train.test and self.config.train.epochs_to_next_test > 0 and epoch % self.config.train.epochs_to_next_test == 0:
                ...
                if self.config.train.val_split == "sequences":
                    train_dataset_sub, val_dataset_sub = dataset.split_sequences(1. - self.config.train.val_fraction, self.config.train.val_fraction)
                elif self.config.train.val_split == "frames":
                    train_dataset_sub, val_dataset_sub = dataset.split_frames(1. - self.config.train.val_fraction, self.config.train.val_fraction)

                for dataset_val in datasets_val + [val_dataset_sub]:
                    results_val = self.test(dataset_val)
                    wandb.log({f'val/{dataset_val.name}/{k}': v for k, v in results_val.items()})


            visual_names_unique = [dataset_sub[i].name_unique for i in range(self.config.train.visualize.num_samples)]
            dataset_train_seq_filtered, dataset_val_seq_filtered = torch.utils.data.random_split(dataset_sub,[1. - self.config.train.val_fraction, self.config.train.val_fraction], generator=generator)
            ...
            if epoch % self.config.train.epochs_to_next_val == 0:
    ...
    def train_epoch(self, dataloader_train, epoch):
        for i, batch in enumerate(iter(dataloader_train)):
            results_batch = self.train_batch(batch=batch)
            wandb.log({'train/' + k: v for k, v in results_batch.items()})

        self.scheduler.step()
        if (epoch + 1) % self.config.train.epochs_to_next_ckpt == 0:
            self.save_checkpoint(path_checkpoint=self.logging_dir.joinpath('nemo.ckpt'))
        self.optim.zero_grad()

    def visualize_train_epoch(self, train_results):
        #self.config.train.visualize.num_samples
        visual_ids = []
        visual_descr = []
        #train_results['loss']
        #train_results['name_unique']
    ...
    def inference_batch(self, batch, config: DictConfig, visual_names_unique=None):
        ...
        if config.sample.method == 'uniform':
            azim = torch.linspace(start=eval(config.azim.min), end=eval(config.azim.max), steps=config.azim.steps).to(
            ...
            theta = torch.linspace(start=eval(config.theta.min), end=eval(config.theta.max),
                                   steps=config.theta.steps).to(
                device=self.device)  # -torch.pi / 6, end=torch.pi / 6, steps=3
            dist = torch.linspace(start=eval(config.dist.min), end=eval(config.dist.max), steps=config.dist.steps).to(
                device=self.device)
        ...
        with torch.no_grad():
            ...
            if config.sample.method == 'epnp3d2d':
                sim_nearest_texture_vals, sim_nearest_texture_ids = torch.einsum('bchw,bvc->bvhw', net_feats2d,
                                                                                 self.meshes.get_feats_stacked_with_mesh_ids(
                                                                                     pred_class_ids).detach()).max(
                    dim=1)
                sim_clutter = \
                torch.einsum('bchw,nc->bnhw', net_feats2d, self.clutter_feats.detach()).max(dim=1, keepdim=False)[0]
                # sim_clutter = torch.einsum('bchw,nc->bnhw', net_feats2d, self.clutter_feats.detach()).mean(dim=1, keepdim=False)

                sim_nearest_texture_verts = torch.stack([self.meshes.get_verts_stacked_with_mesh_ids(
                    pred_class_ids[b: b + 1])[0, sim_nearest_texture_ids[b]] for b in range(B)], dim=0)
                sim_nearest_texture_verts2d = get_pxl2d_like(
                    sim_nearest_texture_verts)  # H=sim_clutter.shape[1], W=sim_clutter.shape[2], dtype=sim_nearest_texture_verts.dtype, device=sim_nearest_texture_verts.device)[None,].expand()

                K = config.sample.epnp3d2d.count_cams
                N = config.sample.epnp3d2d.count_pts
                prob_well_corresp = (sim_clutter < sim_nearest_texture_vals).flatten(
                    1) * sim_nearest_texture_vals.flatten(1)
                ...
                masks_in = torch.zeros(size=(B, K, sim_clutter.shape[1] * sim_clutter.shape[2]),
                                       device=sim_clutter.device, dtype=torch.bool)
                for b in range(B):
                    for k in range(K):
                ...
                b_cams_multiview_tform4x4_obj = batchwise_fit_se3_to_corresp_3d_2d_and_masks(masks_in=masks_in,
                                                                                             pts1=sim_nearest_texture_verts.permute(
                                                                                                 0, 3, 1, 2),
                                                                                             pxl2=sim_nearest_texture_verts2d.permute(
                                                                                                 0, 3, 1, 2),
                                                                                             proj_mat=batch.cam_intr4x4[
                                                                                                      :, :2,
            ...
            if config.visualize.samples:
                for b in range(len(batch)):
                    if visual_names_unique is not None and batch.name_unique[b] in visual_names_unique:
                        ...
                        if config.sample.method == 'uniform':
                            imgs = imgs.reshape(config.azim.steps, config.elev.steps, config.theta.steps,
                                                *imgs.shape[-3:])[:, :, 0]
                            imgs_sim = imgs_sim.reshape(config.azim.steps, config.elev.steps, config.theta.steps)[:, :,
                                       0]
        ...
        if config.pose_iterative_refine:
            ...
            for epoch in range(config.optimizer.epochs):
                ...
                sim, sim_pxl = self.get_sim(feats2d_net=net_feats2d, feats2d_rendered=mesh_feats2d_rendered,
                                            return_sim_pxl=True)  # , sim_clutter_only_outside_render=True)
                ...
                if config.visualize.live:
                    show_img(
                        blend_rgb(batch.rgb[0], (self.meshes.render_feats(cams_tform4x4_obj=cam_transf4x4_obj[0:0 + 1],
            ...
            for b in range(len(batch)):
                if visual_names_unique is not None and batch.name_unique[b] in visual_names_unique:
                    img = blend_rgb(batch.rgb[b], (
                        self.meshes.render_feats(cams_tform4x4_obj=cam_transf4x4_obj[b:b + 1],
                                                 cams_intr4x4=batch.cam_intr4x4[b:b + 1],
    ...
    def test(self, dataset: OD3D_Dataset, config_inference: DictConfig = None, pose_iterative_refine=True,
        ...
        dataloader = torch.utils.data.DataLoader(dataset=dataset_sub, batch_size=self.config.test.dataloader.batch_size,
                                                 shuffle=False,
                                                 collate_fn=dataset.collate_fn,
        ...
        for i, batch in tqdm(enumerate(iter(dataloader))):
            ...
            _, _, results_batch = self.inference_batch(batch=batch, config=config_inference,
                                                       visual_names_unique=visual_names_unique)
