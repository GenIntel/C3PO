...
from od3d.cv.cluster.spectral_clustering import spectral_clustering, spectral_clustering_mean_shift
...
def plot_shared_region3D(tensor1, tensor2, path):
    ...
    ax.scatter(vectors1[:, 0], vectors1[:, 1], vectors1[:, 2], color='blue', label='Ref seq')

    # Scatter plot of vectors from the second tensor
    ax.scatter(vectors2[:, 0], vectors2[:, 1], vectors2[:, 2], color='red', label='Src seq')
    print('start')
    # Optionally, add lines to visualize them better
    for v in vectors1:
        #print('v', v)
        ax.quiver(0, 0, 0, v[0], v[1], v[2], length=1.0, normalize=True, arrow_length_ratio=0.05, color='blue')

    for v in vectors2:
        ax.quiver(0, 0, 0, v[0], v[1], v[2], length=1.0, normalize=True, arrow_length_ratio=0.05, color='red')
...
def plot_shared_region2D(tensor1, tensor2, path):
    ...
    # Plotting
    fig, ax = plt.subplots()
    ax.quiver(np.zeros_like(vectors1[:, 0]), np.zeros_like(vectors1[:, 1]), vectors1[:, 0], vectors1[:, 1],
              color='blue', scale=1, scale_units='xy', angles='xy', label='Ref Seq')
    ax.quiver(np.zeros_like(vectors2[:, 0]), np.zeros_like(vectors2[:, 1]), vectors2[:, 0], vectors2[:, 1],
              color='red', scale=1, scale_units='xy', angles='xy', label='Src Seq')
    ax.set_xlim(-1.1, 1.1)
    ax.set_ylim(-1.1, 1.1)
    ax.set_aspect('equal')
    # Title, legend, and grid
...
def plot_vectors2D(tensor1, tensor2, path):
    ...
    fig, ax = plt.subplots()
    ax.quiver(np.zeros_like(vectors1[:, 0]), np.zeros_like(vectors1[:, 1]), vectors1[:, 0], vectors1[:, 1],
              color='blue', scale=1, scale_units='xy', angles='xy', label='Tensor 1 Vectors')
    ax.quiver(np.zeros_like(vectors2[:, 0]), np.zeros_like(vectors2[:, 1]), vectors2[:, 0], vectors2[:, 1],
              color='red', scale=1, scale_units='xy', angles='xy', label='Tensor 2 Vectors')
...
def apply_rowwise_softmax_excluding_diagonal(weighted_matrics, temperature):
    ...
    diagonal_mask = torch.eye(weighted_matrics.shape[0], dtype=torch.bool).to(weighted_matrics.device)
    ...
    weighted_matrics = torch.nn.functional.softmax(weighted_matrics / temperature, dim=1)
            ...
            for cat_id, category in enumerate(categories):
                ...
                for r, ref_mesh_id in enumerate(ref_mesh_ids):
                    ...
                    for s, src_mesh_id in enumerate(src_mesh_ids):
                        ...
                        pts3d_src, pts3d_colors_src, pts3d_normals_src = src_seq_pcl.read_pcl()
                        if r > 0 and (
                            ...
                            if self.config.global_optimization_steps > 1 and i > 0:
                                ...
                                sequences_annotated = list(dataset_ref.dict_nested_frames_annotated[category].keys())
                                ...
                                for i in range(len(category_dict_ref_seqs[category])):
                                    if category_dict_ref_seqs[category][i] in sequences_annotated:
                                        annotated_idx_list.append(i)
                                use_sph = src_sequences[src_mesh_id].use_sph
                                sfm_type_for_saving = src_sequences[src_mesh_id].sfm_type
                                pcl_type_for_saving = src_sequences[src_mesh_id].pcl_type
                                ...
                                ref_mesh_feats_acc = ref_sequences[ref_mesh_id].read_mesh_feats()
                                ref_sph_feat_cluster_mean, ref_original_indices, ref_mesh_feat_filtered = preprocess_mesh_feats_acc(ref_mesh_feats_acc)
                                pts_ref = pts_ref[ref_original_indices].cuda()

                                src_mesh_feats_acc = src_sequences[src_mesh_id].read_mesh_feats()
                                src_sph_feat_cluster_mean, src_original_indices, src_mesh_feat_filtered = preprocess_mesh_feats_acc(src_mesh_feats_acc)
                                pts_src = pts_src[src_original_indices].cuda()

                                dist_ref_src = dist_ref_src[ref_original_indices, :][:, src_original_indices]

                                weight_selected_model =  0
 
                                if r!=s:
                                    ref_indices, src_indices = get_shared_region_mesh_vertex_feat_cluster_mean(ref_sph_feat_cluster_mean, src_sph_feat_cluster_mean, threshold)
                                    ref_indices = torch.unique(torch.tensor(ref_indices))
                                    src_indices = torch.unique(torch.tensor(src_indices))
                                    if len(ref_indices) == 0 or len(src_indices) == 0:
                                        weight_selected_model = -1.0
                                        src_tform4x4_ref = torch.eye(4)
                                        continue

                                    dist_ref_src = dist_ref_src[ref_indices, :][:, src_indices]
                                    pts_ref = pts_ref[ref_indices].cuda()
                                    pts_src = pts_src[src_indices].cuda()

                                    ref_sph_feat_cluster_mean = filter_mesh_feats_acc(ref_sph_feat_cluster_mean, ref_indices)
                                    ref_mesh_feat_filtered = filter_mesh_feats_acc(ref_mesh_feat_filtered, ref_indices)

                                    src_sph_feat_cluster_mean = filter_mesh_feats_acc(src_sph_feat_cluster_mean, src_indices)
                                    src_mesh_feat_filtered = filter_mesh_feats_acc(src_mesh_feat_filtered, src_indices)


                                src_tform4x4_ref, models, scores, best_correspondence, best_ref_correspondence, best_geo_dist, best_appear_dist, src_tform4x4_ref_score, pts_ids, pts_ref_ids,proposal_dist_ref_geo_avg, proposal_dist_ref_appear_avg  = ransac(
                                    pts=pts_ref,
                                    fit_func=partial(
                                        fit_tform4x4,
                                        pts_ref=pts_src,
                                ...
                                from od3d.cv.visual.correspondences_vis import save_visualization_mesh, convert_pts_mesh, save_pgo_init_meshes, save_pgo_init_meshes_transformed, vis_heatmap_from_vertices, get_closest_correspondence_model, save_visualization_mesh_with_color, compare_sph_dino,compare_sph_dino_best_id, MeshVisualizer
                                ...
                                transformed_pts_src =((inv_tform4x4(src_tform4x4_ref)) @ torch.cat((pts_src,torch.ones(pts_src.size(0),1).cuda()), dim=1).T).T
                                closest_indices_in_src = find_closest_vertices(pts_ref, transformed_pts_src[:,:3])

                                value_list = []
                                for i in range(len(pts_ref)):
                                   value = get_max_cosine_similarity_vertex_pairs(ref_sph_feat_cluster_mean[i], src_sph_feat_cluster_mean[closest_indices_in_src[i] ])
                                   value_list.append(value)

                                weight_selected_model = torch.mean(torch.tensor(value_list, dtype = torch.float32))
                                
                                transformation_score = (1/(-src_tform4x4_ref_score.item()))
                                ...
                                logger.info(f'The ESTIMATE of RANSAC is {weight_ransac} ')
                                logger.info(f'THE ESTIMATE of SELECTED MODEL is {weight_selected_model}')
                                logger.info(f'THE TRANSFORMATION SCORE IS {transformation_score}' )
                                
                                logger.info(f'THE FINAL WEIGHT IS {final_weight}')
                                
                                exp_name = self.config.name
                                vi_mesh_path = '/CT/3D_DST_Scene/work/od3d/scripts/vis_meshes'
                                vi_mesh_path = os.path.join(vi_mesh_path, f'{exp_name}')
                                os.makedirs(vi_mesh_path, exist_ok = True)
                                vi_mesh_path = os.path.join(vi_mesh_path, f'ransac_estimation_{self.config.weight.ransac_estimation}_transformation_score_{self.config.weight.transformation_score}_sph_{self.config.weight.sph_weight}')
                                os.makedirs(vi_mesh_path, exist_ok = True)
                                category_path = os.path.join(vi_mesh_path, f'category_{category}')
                                os.makedirs(category_path, exist_ok=True)
                                
                                ratio_path = os.path.join(category_path, f'ratio_{int(100 *partial_ratio_for_saving )}')
                                ...
                                feature_path = os.path.join(threshold_path,f'use_sph_{use_sph}_mixing_ratio_{mixing_ratio}')
                                # if os.path.exists(feature_path):
                                #     shutil.rmtree(feature_path)
                                os.makedirs(feature_path, exist_ok= True)
                                category_ratio_use_sph_path = os.path.join(feature_path, f'id_start_{ref_start}')
                                os.makedirs(category_ratio_use_sph_path, exist_ok= True)
                                sfm_pcl_type_path = os.path.join(category_ratio_use_sph_path,f'sfm_type_{sfm_type_for_saving}_pcl_type_{pcl_type_for_saving}' )
                                os.makedirs(sfm_pcl_type_path, exist_ok= True)
                                folder_path = os.path.join(sfm_pcl_type_path, f'ref_id_{ref_name}_src_id_{src_name}_aligned')
                                os.makedirs(folder_path, exist_ok= True)
                                os.makedirs(os.path.join(sfm_pcl_type_path, f'ref_id_{ref_name}_src_id_{src_name}'), exist_ok=True)

                                if r!=s: 
                                    mesh_vis_folder_path_before_alignment = os.path.join(sfm_pcl_type_path, f'ref_id_{ref_name}_src_id_{src_name}', 'before_alignment')
                                    os.makedirs(mesh_vis_folder_path_before_alignment, exist_ok = True)
                                    save_visualization_mesh(pts= pts_src, pts_ref= pts_ref, pts_ids= best_correspondence, pts_ref_ids= best_ref_correspondence, filename= os.path.join(sfm_pcl_type_path, f'ref_id_{ref_name}_src_id_{src_name}'))
                                    
                                    pts_ref_o3d = open3d.geometry.PointCloud()
                                    pts_cloned = pts3d.clone().cpu().numpy()
                                    pts_color_cloned = pts3d_colors.clone().cpu().numpy()
                                    pts_ref_o3d.points = open3d.utility.Vector3dVector(pts_cloned)
                                    pts_ref_o3d.colors = open3d.utility.Vector3dVector(pts_color_cloned)
                                    
                                    pts_src_o3d = open3d.geometry.PointCloud()
                                    pts_cloned = pts3d_src.clone().cpu().numpy()
                                    pts_color_cloned = pts3d_colors_src.clone().cpu().numpy()
                                    pts_src_o3d.points = open3d.utility.Vector3dVector(pts_cloned)
                                    pts_src_o3d.colors = open3d.utility.Vector3dVector(pts_color_cloned)
                                    
                                    
                                    mesh_vis = MeshVisualizer(pts_ref_o3d, pts_src_o3d, pts_src[best_ref_correspondence], pts_ref[best_correspondence], mesh_vis_folder_path_before_alignment )
                                    mesh_vis.save()
                                    #MeshVisualizer(pts_ref_o3d, pts_src_o3d.transform(nv_tform4x4(src_tform4x4_ref)), )
                                    tmp_transformation = inv_tform4x4(src_tform4x4_ref).cpu().numpy()
                                    transformed_pts = pts_src_o3d.transform( tmp_transformation )
                                    mesh_vis_folder_path_after_alignment = os.path.join(sfm_pcl_type_path, f'ref_id_{ref_name}_src_id_{src_name}', 'after_alignment')
                                    os.makedirs(mesh_vis_folder_path_after_alignment, exist_ok = True)
                                    open3d.io.write_point_cloud(os.path.join(mesh_vis_folder_path_after_alignment, 'tranformed_src_pts.ply'),transformed_pts )
                                    open3d.io.write_point_cloud(os.path.join(mesh_vis_folder_path_after_alignment, 'ref_pts.ply'), pts_ref_o3d )

                                    save_visualization_mesh(pts= transformed_pts_src[:,:3], pts_ref= pts_ref, pts_ids= best_correspondence, pts_ref_ids= best_ref_correspondence, filename= folder_path)
                                ...
                                normalized_rotation, scale = extract_rotation_from_sim3(pred_ref_tform_src)
                        ...
                        logger.info(f'diff rot degree for the baseline is {180 * diff_rot_angle_rad / torch.pi}')
                        print('------------------------------------------------------------------------------------------------------------')
                        results_diff_log_rot[category][r, s] = diff_rot_angle_rad
         
                        if r != s:
                            rot_deg_diff_list.append(180 * diff_rot_angle_rad / torch.pi)   
                            
                from od3d.cv.optimization.masked_posegraph import WeightedPoseGraphOptimizationRotationMatrixMasking, WeightedPoseGraphOptimizationRotation
                os.makedirs(os.path.join(feature_path, 'pgo_init'), exist_ok = True )
                torch.save(all_pred_ref_tform_src_rotational[category], os.path.join(feature_path, 'pgo_init', 'input_alignments.pt'))
                weighted_matrics = weighted_matrix[category].clone()
                torch.save(weighted_matrics, os.path.join(feature_path, 'pgo_init', f'weighted_matrics.pt'))
                
                show_weight = weighted_matrics[annotated_idx_list, :][:, annotated_idx_list]
                ...
                #         f.write("%s\n" % item)
                with open(os.path.join(feature_path, 'pgo_init', f'seq_list.json'), 'w') as json_file:
                ...
                weighted_matrics = apply_softmax_entire_matrix(weighted_matrics, temperature )
                ...
                torch.save(torch.tensor(annotated_idx_list),  os.path.join(feature_path, 'pgo_init', 'annotated_idx.pt') )
                print('result before pgo ', results_diff_log_rot[category][annotated_idx_list,:][:,annotated_idx_list ] * 180 / torch.pi)
               
                print('The weighted_matrices',weighted_matrics[annotated_idx_list, :][:, annotated_idx_list].shape)
                ...
                pgo = WeightedPoseGraphOptimizationRotationMatrixMasking(all_pred_ref_tform_src_rotational[category], weighted_matrices= weighted_matrics, seq_list =  ref_sequences_unique_names)
                ...
                save_pgo_init_meshes_transformed(pose_node, pts_list, pts_color_list, pgo_folder, name_list)
                
                pgo_folder_optimized = os.path.join(feature_path, 'pgo_optimized')
                save_pgo_init_meshes_transformed(pose_node_optimized, pts_list, pts_color_list, pgo_folder_optimized, name_list)
                ...
                print('pose graph intialization ', pose_graph_init_transformations[category][annotated_idx_list, :][:, annotated_idx_list]* 180.0 / torch.pi )
                

                bac = BaselineAlignmentToCanonicalization(all_pred_ref_tform_src_rotational[category])
                baseline_results = bac.convert()
                for baseline_results_idx in range(len(baseline_results)):
                    baseline_results_folder = os.path.join(feature_path, f'baseline_{baseline_results_idx}')
                    save_pgo_init_meshes_transformed(baseline_results[baseline_results_idx], pts_list, pts_color_list, baseline_results_folder, name_list)

                os.makedirs(os.path.join(pgo_folder, 'node_order'), exist_ok= True)
                for i in range(len(node_order)):
                    vertices_mask = (
                        ref_sequences_mesh_ids_for_verts == int(node_order[i])
                    )
                    pts_ref = ref_meshes.verts[vertices_mask].clone()

                    convert_pts_mesh(pts_ref, os.path.join(pgo_folder, 'node_order',f'order_{i}.ply' ))
                    
                pose_graph_results_diff_log_rot[category] = updated_rad
                print('pose_graph_results_diff_log_rot ', pose_graph_results_diff_log_rot[category][annotated_idx_list, :][:, annotated_idx_list]* 180.0 / torch.pi )
                ...
                result_dict[category]['pi_6_accuracy_pg_init'] = category_results_mean[f'pose/prefix/{category}_pose_graph_init_acc_pi6']
                result_dict[category]['pi_12_accuracy_pg_init'] = category_results_mean[f'pose/prefix/{category}_pose_graph_init_acc_pi12']
                result_dict[category]['pi_18_accuracy_pg_init'] = category_results_mean[f'pose/prefix/{category}_pose_graph_init_acc_pi18']
                result_dict[category]['error_mean_pg_init'] = category_results_mean[f'pose/prefix/{category}_pose_graph_init_err_mean']
                result_dict[category]['error_median_pg_init'] = category_results_mean[f'pose/prefix/{category}_pose_graph_init_err_median']
