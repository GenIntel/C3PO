...
class NeMo_DINO(NeMo):
    ...
    def train_batch(self, batch) -> OD3D_Results:
        ...
        batch_vts_ids = self.meshes.get_verts_and_noise_ids_stacked(
        ...
        if self.config.train.bank_feats_update == "loss_gradient":
            ...
            sim = self.calc_sim("nc,vc->nv", net_feats, bank_feats)
            bank_feats_new = (
                self.config.train.alpha * bank_feats[batch_vts_ids].detach()
                + (1.0 - self.config.train.alpha) * net_feats.detach()
            )
            (
                batch_vts_ids_unique,
                batch_vts_ids_unique_inverse,
                batch_vts_ids_unique_counts,
            ) = batch_vts_ids.unique(return_inverse=True, return_counts=True)
            bank_feats_new = (
                torch.einsum(
                    "nk,nc->kc",
            ...
            bank_feats[batch_vts_ids_unique].data = (
