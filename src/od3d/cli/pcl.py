...
def show_mesh():
    ...
    meshes = Meshes.load_from_files(
        fpaths_meshes=fpaths_meshes,
        fpaths_meshes_tforms=fpaths_meshes_tform_obj,
    )

    import torch

    tform_obj = torch.load(
        f"/misc/lmbraid19/sommerl/datasets/CO3D_Preprocess/tform_obj/label3d_cuboid/meta_mask/meta/bicycle/{sequence_name}/tform_obj.pt",
    )
    tform_obj = torch.load(
        f"/misc/lmbraid19/sommerl/datasets/CO3D_Preprocess/tform_obj/label3d/meta_mask/meta/bicycle/{sequence_name}/tform_obj.pt",
    )
    ...
    show.show_scene(
        meshes=meshes,
        meshes_colors=meshes.get_verts_ncds_cat_with_mesh_ids(),
    )
...
@app.command()
def show_pts(
    fpath: str = typer.Option(None, "-f", "--fpath"),
    device: str = typer.Option("cpu", "-d", "--device"),
):
    from od3d.cv.io import (
        read_pts3d,
        read_pts3d_with_colors_and_normals,
    )
    ...
    alpha = particle_size / 2.0 * 2
    print(quantile, particle_size, alpha)

    o3d_mesh = open3d.geometry.TriangleMesh.create_from_point_cloud_alpha_shape(
        o3d_pcl,
        alpha,
    )
    show.show_scene(
...
def show_open3d():
    ...
    from kaolin.ops.conversions import (
        pointclouds_to_voxelgrids,
        voxelgrids_to_trianglemeshes,
    )

    # BxNx3
    mesh = voxelgrids_to_trianglemeshes(
        pointclouds_to_voxelgrids(pointclouds=pts3d[None,].cuda(), resolution=256),
    )
    # print(pcd)
