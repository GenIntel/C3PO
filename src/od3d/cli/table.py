...
TABLE_CATEGORIES_YOLO = [
    ...
    "motorcycle",
    "mean (20)",
]  # # B’pack Car Chair Keyboard Laptop M’cycle
...
@app.command()
def runs(
    runs_names_regex: str = typer.Option(".*", "-r", "--runs"),
    ...
    if summary_cols is not None:
        for metric in metrics:
            df_metric = (
                df.groupby(summary_cols.split(","))[metric]
                .agg(["mean", "std"])
                .reset_index()
            )
...
def pose_categories(
    ...
    datasets = [
        ...
        "pascal3d",
        "co3d_20",
    ]  # 'pascal3d' 'co3d_20' 'co3d_28' 'objectnet3d'
    ...
    for dataset in datasets:
        if "cat1" in name_regex:
            ...
            metrics_dfs = get_categorical_results_from_multiple_runs(
                metrics=metrics,
            ...
            metrics_dfs = get_categorical_results_from_single_runs(
                metrics_templates=metrics,
...
def ablation_dist():
    ...
    align3d_1on1_df = get_dataframe(
        ...
        age_in_hours_lt=age_in_hours,
        name_regex=align3d_1on1_name_partial,
    )
    ...
    align3d_1on1_df[cols_dec] *= 100.0
    ...
    logger.info(
        tabulate(df, headers="keys", tablefmt="latex", floatfmt=".1f"),
    )  # 'github', 'tsv', 'latex', 'latex_raw'
    logger.info(
        tabulate(align3d_1on1_df, headers="keys", tablefmt="latex", floatfmt=".1f"),
    )  # 'github', 'tsv', 'latex', 'latex_raw'
...
def get_categorical_results_from_multiple_runs(
    metrics,
    age_in_hours: float,
    configs=[],
    name_regex=f".*cat1_([a-z]*)_ref([0-9]*)_.*",
    metrics_scales=None,
):
    rows = []
    ...
    df = get_dataframe(
        ...
        age_in_hours_lt=age_in_hours,
        name_regex=name_regex,
        name_regex_groups=[COLUMN_CATEGORY, COLUMN_REFERENCE],
        filter_runs_with_metrics=False,
    )
    metrics_dfs = []
    for m, metric in enumerate(metrics):
        ...
        metric_df = metric_df.drop_duplicates(
            subset=[COLUMN_CATEGORY, COLUMN_REFERENCE],
            keep="first",
        )
        metric_df_mean_over_refs = (
            metric_df.groupby(COLUMN_CATEGORY)[metric].mean(numeric_only=False)
            * metric_scale
        )
        # metric_df.groupby(COLUMN_CATEGORY).count()
        metric_df_mean_over_refs[COLUMN_CATEGORY_MEAN] = metric_df_mean_over_refs.mean()
        metric_df_std_over_refs = (
            metric_df.groupby(COLUMN_CATEGORY)[metric].std(numeric_only=False)
            * metric_scale
        )
        metric_df_std_over_refs[COLUMN_CATEGORY_MEAN] = metric_df_std_over_refs.mean()
        metric_df = pd.DataFrame(
            {"mean": metric_df_mean_over_refs, "std": metric_df_std_over_refs},
        ).reset_index()
...
def get_categorical_results_from_single_runs(
    metrics_templates,
    categories,
    age_in_hours: float,
    configs=[],
    name_regex=".*_CO3D_NeMo_ref.*",
    metrics_scales=None,
    ...
    for m, metric in enumerate(metrics_templates):
        ...
        for category in categories:
            if category not in map_od3d_to_dataset:
                map_od3d_to_dataset[
                    category
                ] = category  # f'pose/prefix/{map_od3d_to_dataset[category]}_acc_pi6' # f'pose/prefix/{map_od3d_to_dataset[category]}_acc_pi6_std'
            ...
            metrics.append(
                metric.replace("CATEGORY", map_od3d_to_dataset[category]) + "_std",
            )
        ...
        df = pd.concat(
            [
                df[columns_mean].rename(columns=columns_mean_map),
                df[columns_std].rename(columns=columns_std_map),
            ],
        )
...
def pose_pi6_categories_separate():
    ...
    metrics = [
        "test/pascal3d_test/pose/acc_pi6",
        "test/objectnet3d_test/pose/acc_pi6",
        "test/co3d_no_zsp_5s_labeled_ref/pose/acc_pi6",
        "test/co3dv1_10s_zsp_labeled_cuboid_ref/pose/acc_pi6",
    ]
    ...
    metrics_dfs = get_categorical_results_from_single_runs(
        ...
        name_regex=name_regex,
        metrics_scales=metrics_scales,
        map_od3d_to_datasets=map_od3d_to_datasets,
    )


def tabulate_metrics_for_dataset(
    metrics_dfs,
    datasets: List[str],
    digits=2,
    tablefmt="latex",
    ...
    _TABLE_CATEGORIES_PASCAL3D = list(
        filter(
            lambda category: category not in BAN_CATEGORIES,
            TABLE_CATEGORIES_PASCAL3D,
        ),
    )
    ...
    _TABLE_CATEGORIES_OBJECTNET3D_20 = list(
        filter(
            lambda category: category not in BAN_CATEGORIES,
            TABLE_CATEGORIES_OBJECTNET3D_20,
        ),
    )
    _TABLE_CATEGORIES_OBJECTNET3D_11 = list(
        filter(
            lambda category: category not in BAN_CATEGORIES,
            TABLE_CATEGORIES_OBJECTNET3D_11,
        ),
    )
    _TABLE_CATEGORIES_OBJECTNET3D_9 = list(
        filter(
            lambda category: category not in BAN_CATEGORIES,
            TABLE_CATEGORIES_OBJECTNET3D_9,
        ),
    )
    ...
    _TABLE_CATEGORIES_ZSP = list(
        filter(lambda category: category not in BAN_CATEGORIES, TABLE_CATEGORIES_ZSP),
    )
    _TABLE_CATEGORIES_YOLO = list(
        filter(lambda category: category not in BAN_CATEGORIES, TABLE_CATEGORIES_YOLO),
    )
    if ALLOW_CATEGORIES is not None:
        _TABLE_CATEGORIES_PASCAL3D = list(
            filter(
                lambda category: category in ALLOW_CATEGORIES
                or category.startswith("mean"),
                _TABLE_CATEGORIES_PASCAL3D,
            ),
        )
        _TABLE_CATEGORIES_OBJECTNET3D_20 = list(
            filter(
                lambda category: category in ALLOW_CATEGORIES
                or category.startswith("mean"),
                _TABLE_CATEGORIES_OBJECTNET3D_20,
            ),
        )
        _TABLE_CATEGORIES_OBJECTNET3D_9 = list(
            filter(
                lambda category: category in ALLOW_CATEGORIES
                or category.startswith("mean"),
                _TABLE_CATEGORIES_OBJECTNET3D_9,
            ),
        )
        _TABLE_CATEGORIES_OBJECTNET3D_11 = list(
            filter(
                lambda category: category in ALLOW_CATEGORIES
                or category.startswith("mean"),
                _TABLE_CATEGORIES_OBJECTNET3D_11,
            ),
        )
        _TABLE_CATEGORIES_CO3D_20 = list(
            filter(
    ...
    for m, metric_df in enumerate(metrics_dfs):
        if datasets[m] == DATASET_PASCAL3D:
            ...
            metric_df[_TABLE_CATEGORIES_PASCAL3D[-1]] = [
                metric_df[_TABLE_CATEGORIES_PASCAL3D[:-1]].loc["mean"].mean(),
                metric_df[_TABLE_CATEGORIES_PASCAL3D[:-1]].loc["std"].mean(),
            ]

            logger.info(
                tabulate(
                    metric_df[_TABLE_CATEGORIES_PASCAL3D],
                    headers="keys",
                    tablefmt=tablefmt,
                    floatfmt=f".{digits}f",
                ),
            ...
            zeros_df = pd.DataFrame(
                columns=_TABLE_CATEGORIES_OBJECTNET3D_20[:-1],
                index=["mean", "std"],
                data=0.0,
            )
            metric_df = pd.merge(
                metric_df,
                zeros_df[zeros_df.columns.difference(metric_df.columns)],
                left_index=True,
                right_index=True,
                how="outer",
            )
            logger.info("ObjectNet3D")
            for object_net3d_table in [
                _TABLE_CATEGORIES_OBJECTNET3D_20,
                _TABLE_CATEGORIES_OBJECTNET3D_11,
                _TABLE_CATEGORIES_OBJECTNET3D_9,
            ]:
            ...
            metric_df[_TABLE_CATEGORIES_CO3D_20[-1]] = [
                metric_df[_TABLE_CATEGORIES_CO3D_20[:-1]].loc["mean"].mean(),
                metric_df[_TABLE_CATEGORIES_CO3D_20[:-1]].loc["std"].mean(),
            ]
            logger.info("Dataset: ZSP, Categories: ZSP")
            logger.info(
                tabulate(
                    metric_df[_TABLE_CATEGORIES_ZSP],
                    headers="keys",
                    tablefmt=tablefmt,
                    floatfmt=f".{digits}f",
                ),
            ...
            logger.info(
                tabulate(
                    metric_df[_TABLE_CATEGORIES_YOLO],
                    headers="keys",
                    tablefmt=tablefmt,
                    floatfmt=f".{digits}f",
                ),
            ...
            logger.info(
                f"Dataset: ZSP, Categories: {len(_TABLE_CATEGORIES_CO3D_20[:-1])}",
            )
            logger.info(
                tabulate(
                    metric_df[_TABLE_CATEGORIES_CO3D_20],
                    headers="keys",
                    tablefmt=tablefmt,
                    floatfmt=f".{digits}f",
                ),
            ...
            metric_df[_TABLE_CATEGORIES_CO3D_28[-1]] = [
                metric_df[_TABLE_CATEGORIES_CO3D_28[:-1]].loc["mean"].mean(),
            ...
            logger.info(
                f"Dataset: Ours, Categories: {len(_TABLE_CATEGORIES_CO3D_28[:-1])}",
            )
            logger.info(
                tabulate(
                    metric_df[_TABLE_CATEGORIES_CO3D_28],
                    headers="keys",
                    tablefmt=tablefmt,
                    floatfmt=f".{digits}f",
                ),
...
def pose_pi6_categories_align3d_co3dv1():
    ...
    for category in TABLE_CATEGORIES_CO3D_20[:-1]:
        ...
        columns_mean_map[columns_map[metrics[-1]]] = category
        metrics.append(
            f"pose/prefix/{MAP_CATEGORIES_OD3D_TO_CO3D[category]}_acc_pi6_std",
        )
        columns_map[metrics[-1]] = category + "_std"
        columns_std.append(columns_map[metrics[-1]])
        columns_std_map[columns_map[metrics[-1]]] = category
...
def pose_pi6_categories():
    ...
    for category in categories:
        pascal3d_nemo_metrics.append(
            f"test/pascal3d_test/pose/prefix/{category}_acc_pi6",
        )
        pascal3d_nemo_columns_map[pascal3d_nemo_metrics[-1]] = category
    pascal3d_nemo_df = get_dataframe(
        configs=configs,
        metrics=pascal3d_nemo_metrics,
        age_in_hours_lt=age_in_hours,
        name_regex=pascal3d_nemo_name_partial,
    )
    ...
    for category in categories:
        align3d_metrics.append(
            f"only_to_ref/pose/prefix/{MAP_CATEGORIES_OD3D_TO_CO3D[category]}_acc_pi6",
        )
    ...
    nemo_df = get_dataframe(
        configs=configs,
        metrics=nemo_metrics,
        age_in_hours_lt=age_in_hours,
        name_partial=nemo_name_partial,
    )
    nemo_df = nemo_df.rename(columns=nemo_columns_map)
    align3d_df = get_dataframe(
        configs=configs,
        metrics=align3d_metrics,
        age_in_hours_lt=age_in_hours,
        name_partial=align3d_name_partial,
    )
    ...
    align3d_1on1_df = get_dataframe(
        ...
        metrics=align3d_1on1_metrics,
        age_in_hours_lt=age_in_hours,
        name_partial=align3d_1on1_name_partial,
    )
    align3d_1on1_df = align3d_1on1_df.rename(columns=align3d_1on1_columns_map)
    ...
    df = df[cols]
    logger.info(
        tabulate(df, headers="keys", tablefmt="latex", floatfmt=".3f"),
    )  # 'github', 'tsv', 'latex', 'latex_raw'
