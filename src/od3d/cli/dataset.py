...
def visualize_selfsup(
    ...
    for batch in iter(dataloader):
        ...
        feats = (
            model(batch.rgb)
            .permute(0, 2, 3, 1)
            .reshape(-1, model.out_dim)[mask.flatten() > 0.5]
        )
    ...
    for batch in iter(dataloader):
        ...
        feats_pca = (
            ...
            .reshape(feats.shape[0], feats.shape[2], feats.shape[3], pca_dim)
            .permute(0, 3, 1, 2)
        )
...
def visualize_category_frames(
    ...
    config = od3d.io.load_hierarchical_config(
        platform=platform,
        overrides=["+datasets@dataset=" + dataset],
    )
    dataset = OD3D_Dataset.subclasses[config.dataset.class_name].create_from_config(
        config=config.dataset,
    )
    dataset.visualize_category_frames(
...
def visualize_category_pcls(
    ...
    dataset.visualize_category_pcls(
        viewpoints_count=viewpoints_count,
        H=height,
        W=width,
    )
...
def visualize_category_meshes(
    ...
    modalities: str = typer.Option(
        "ncds,nn_geo,nn_app,nn_cycle,cycle_weight,nn_app_cycle_weight",
        "-m",
        "--modalities",
    ),
):
...
@app.command()
def sequences(
    dataset: str = typer.Option("co3d", "-d", "--dataset"),
    ...
    if dataset_ban is not None:
        config = od3d.io.load_hierarchical_config(
            platform=platform,
            overrides=["+datasets@dataset=" + dataset_ban],
        )
        dataset_ban = OD3D_Dataset.subclasses[
            config.dataset.class_name
        ].create_from_config(config=config.dataset)
        sequences_names = list(
            filter(
                lambda sname: sname not in dataset_ban.sequences_names,
                sequences_names,
            ),
        )
    sequences_names_as_str = "\n  - ".join(
        sequences_names,
    )  # sequence.pcl_quality_score
...
def rsync_raw(
    ...
    logging.basicConfig(level=logging.INFO)
    config_source = od3d.io.load_hierarchical_config(
        platform=platform_source,
        overrides=["+datasets@dataset=" + dataset],
    )
    config_target = od3d.io.load_hierarchical_config(
    ...
    od3d.io.run_cmd(
        cmd=f"rsync -avrzP --delete {source_link}{paths_source} {target_link}{paths_target.parent}",
        live=True,
        logger=logger,
    )
...
def visualize_categories(
    dataset: str = typer.Option("coco", "-d", "--dataset"),
    platform: str = typer.Option("local", "-p", "--platform"),
    ...
    from od3d.datasets.co3d.enum import (
        CO3D_CATEGORIES,
    ...
    if map_co3d_to_dataset is not None:
        dataset_categories = [
            map_co3d_to_dataset[cat]
            for cat in co3d_categories
            if map_co3d_to_dataset[cat] is not None
        ]
    ...
    dict_imgs_stacked = dataset.get_frames_categories(
        max_frames_count_per_category=frames_count_per_category,
    )
    ...
    if map_co3d_to_dataset is not None:
        ...
        for co3d_cat in co3d_categories:
            if map_co3d_to_dataset[co3d_cat] is not None:
                dict_imgs_stacked_remapped[co3d_cat] = dict_imgs_stacked[
                    map_co3d_to_dataset[co3d_cat]
                ]
    ...
    for i, category in enumerate(all_categories):
        if i + 1 < len(all_categories):
            ...
            text = category + f"\n {len(all_categories)}"
        img_category_text = draw_text_as_img(
            H=H,
            W=W,
            text=text,
            fontScale=0.6,
        ...
        imgs_place_holders = torch.zeros(
            size=(frames_count_per_category - len(imgs_category), 3, H, W),
            dtype=dtype,
            device=device,
        )
        imgs.append(
            torch.cat(
                [img_category_text[None,], imgs_category, imgs_place_holders],
                dim=0,
            ),
        )
...
def label_add_local(
    ...
    cmd = (
        "curl -X POST http://localhost:8080/api/storages/localfiles/"
        + str(map_name_to_id[name])
        + "/sync/ -H Content-Type:application/json -H 'Authorization: Token 3b92be7993e450f032f653b15c03f157162513cc' --data '{"
        + data_arg
        + "}'"
    )
    run_cmd(cmd, live=True, logger=logger)
...
def label_export(
    ...
    path_labelstudio_labels = dataset.path_preprocess.joinpath(
        "labels",
        "kpts2d_orient",
    )
    ...
    for frame in frames.values():
            ...
            for annot in annots["result"]:
                ...
                if count_kpts % 2 == 1:
                    ...
                    kpoint_pair.append(
                        torch.Tensor(
                            [
                                annot["value"]["x"] / 100.0 * annot["original_width"],
                                annot["value"]["y"] / 100.0 * annot["original_height"],
                            ],
                        ),
                    )
                elif count_kpts % 2 == 0:
                    ...
                    if kpoint2_name != kpoints_pairs[kpoint1_name]:
                        logger.warning(
                            f"Expected kpoint {kpoints_pairs[kpoint1_name]}, got {kpoint2_name}. Skipping kpoint...",
                        )
                        continue
                    kpoint_pair.append(
                        torch.Tensor(
        ...
        for orient in l_orients_kpoints_pairs.keys():
            if len(l_orients_kpoints_pairs[orient]) > 0:
                l_orients_kpoints_pairs[orient] = torch.stack(
                    l_orients_kpoints_pairs[orient],
                    dim=0,
                )
            else:
                l_orients_kpoints_pairs[orient] = torch.Tensor(
                    l_orients_kpoints_pairs[orient],
                )
...
# @app.command()
def label_export_project(
    project_name: str,
):  # = typer.Option(None, '-p', '--project')):
    logging.basicConfig(level=logging.INFO)
    ...
    frames_names = [
        Path(frame["data"]["image"]).name.split("-")[-1] for frame in export_data
    ]
...
def label_start(
    ...
    path_labelstudio_labelconfig = dataset.path_preprocess.joinpath(
        "labelstudio",
        f"label_{category}_config.xml",
    )
...
@app.command()
def label_start_finish(
    project_name: str = typer.Option(None, "-p", "--project"),
    path_labelstudio_in: str = typer.Option(None, "-i", "--path-labelstudio-in"),
    basis_project_name: str = typer.Option(
        "co3d_only_first_car",
        "-b",
        "--basis-project",
    ),
    ...
    if path_labelstudio_in is None:
        path_labelstudio_in = Path(
            "/misc/lmbraid19/sommerl/datasets/CO3D_Preprocess/labelstudio/in",
    ...
    if local_storage_default_name not in local_storages_keys:
        ...
        cmd = (
            "curl -X POST http://localhost:8080/api/storages/localfiles/"
            + " -H Content-Type:application/json -H 'Authorization: Token 3b92be7993e450f032f653b15c03f157162513cc' --data '{"
            + data_arg
            + "}'"
        )
        run_cmd(cmd, live=True, logger=logger)
        ...
        for path in path_labelstudio_in.iterdir():
            ...
            if path.suffix == ".png" or path.suffix == ".jpg":
                if basis_frames is not None and path.name in basis_frames.keys():
                    ...
                    frame = {}
                frame["data"] = {
                    "image": f"/data/local-files/?d=in/{project_name}/{path.name}",
                }
