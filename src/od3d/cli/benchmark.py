...
from od3d.benchmark.run import (
    bench_single_method_local,
    bench_single_method_local_separate_venv,
    bench_single_method_local_docker,
    torque_run_method_or_cmd,
    slurm_run_method_or_cmd,
)
import json
...
def get_timestamp_from_string(string):
    ...
    timestamp = datetime.datetime.strptime(
        "_".join(f"{year}-{string}".split("_")[:2]),
        "%Y-%m-%d_%H-%M-%S",
    )
    return timestamp
...
def get_runs_multiple(
    benchmark: str = None,
    platform: str = None,
    ablation: str = None,
    age_in_hours_gt: int = 0,
    age_in_hours_lt: int = 1000,
    state=None,
):
    run_name_regex = get_run_name_regex(
        ablation=ablation,
        platform=platform,
        benchmark=benchmark,
    )
    return get_runs(
        name_regex=run_name_regex,
...
def get_runs(name_regex=".*", age_in_hours_gt=0, age_in_hours_lt=1000, state=None):
    ...
    timestamp_created_lt = (
        datetime.datetime.now(datetime.timezone.utc)
        - datetime.timedelta(hours=age_in_hours_gt)
    ).isoformat()
    timestamp_created_gt = (
        datetime.datetime.now(datetime.timezone.utc)
        - datetime.timedelta(hours=age_in_hours_lt)
    ).isoformat()
...
def get_dataframe_multiple(
    ablation: str = None,
    platform: str = None,
    benchmark: str = None,
    age_in_hours_gt=0,
    age_in_hours_lt=1000,
    configs=None,
    metrics=[],
    duplicates_keep="all",
    state=None,
    add_configs_ablation=True,
    ...
    if add_configs_default:
        configs += [
            "platform.link",
            "train_datasets.labeled.class_name",
            "method.class_name",
        ]
...
def get_dataframe(
    ...
    age_in_hours_lt=1000,
    age_in_hours_gt=0,
    name_partial_ban=None,
    filter_runs_with_metrics=True,
    duplicates_keep="all",
    state=None,
):
    # Initialize wandb
    ...
    if filter_runs_with_metrics and metrics is not None:
        ...
        runs = list(
            filter(
                lambda run: all(
                    [metric in list(run.summary.keys()) for metric in metrics],
                ),
                runs,
            ),
        )
    ...
    if duplicates_keep is not None and (
        duplicates_keep == "first" or duplicates_keep == "last"
    ):
...
def table():
    ...
    metrics = [
        "test/pascal3d_test/pose/acc_pi6",
        "test/pascal3d_test/pose/acc_pi18",
        "test/pascal3d_test/pose/err_median",
        "test/pascal3d_test/pose/err_mean",
        "test/pascal3d_test/time_pose",
    ]
...
def table_multiple_sequences():
    ...
    configs = [
        "method.class_name",
        "train_datasets.labeled.categories",
        "method.value.multiview.type",
        "method.value.multiview.batch_size",
        "train_datasets.labeled.dict_nested_frames",
    ]
    age_in_hours = 24 * 10
    ...
    my_df = get_dataframe(
        ...
        age_in_hours_lt=age_in_hours,
        name_partial=name_partial,
        name_partial_ban=name_partial_ban,
    )

    my_df["sequence_nth"] = (
        my_df["Run"].str.split("_1s_").str[1:2].str.join("_").str[:3]
    )
    my_df["type"] = (
        my_df["Run"].str[14:].str.split("_1s_").str[0]
        + my_df["Run"].str.split("_1s_").str[1:2].str.join("_").str[3:]
    )
    ...
    for i, metric in enumerate(metrics_new_names):
        ...
        plt.savefig(
            f"seqs_type_{metrics[i].replace('/', '_')}.png",
            bbox_inches="tight",
        )
        plt.clf()
...
def table_multiple_categories_multiview_incremental():
    ...
    # metrics = ['test/co3d_50s_test/pose/acc_pi6', 'test/co3d_50s_test/pose/acc_pi18', 'test/co3d_50s_test/pose/err_median', 'test/co3d_50s_test/pose/err_mean']
    metrics = [
        "test/co3d/pose/acc_pi6",
        "test/co3d/pose/acc_pi18",
        "test/co3d/pose/err_median",
        "test/co3d/pose/err_mean",
    ]
    ...
    my_df["train_datasets.labeled.categories"] = my_df[
        "train_datasets.labeled.categories"
    ].str[0]
    my_df = my_df.groupby(
        ["train_datasets.labeled.categories", "method.class_name"],
    ).head(3)
...
def save_category_sequence_images_as_one(df: pandas.DataFrame):
    ...
    sequences_name = [
        get_first_non_none_value(config)
        for config in df["train_datasets.labeled.dict_nested_frames"]
    ]
    sequences_path_imgs = [
        Path(config.platform_local.path_datasets).joinpath(
            "CO3D",
            sequence_name,
            "images",
        )
    ...
    sequences_fpath_first_imgs = [
        sorted(
            list(sequence_path_imgs.iterdir()),
            key=lambda f: [OD3D_Meta.atoi(val) for val in re.split(r"(\d+)", f.stem)],
        )
        if sequence_path_imgs is not None
        else None
        for sequence_path_imgs in sequences_path_imgs
    ]
    ...
    for i, sequence_fpath_first_imgs in enumerate(
        df_category_sequence_unique["sequences_fpath_first_imgs"],
    ):
        ids = np.linspace(0, len(sequence_fpath_first_imgs) - 1, 3, dtype=int)
        category_sequence_rgbs.append(
            draw_text_in_rgb(
                fpaths_to_rgb(
                    fpaths=[sequence_fpath_first_imgs[id] for id in ids],
                    H=512,
                    W=512,
                ),
    ...
    show_img(
        imgs_to_img(torch.stack(category_sequence_rgbs, dim=0)[:, None]),
        fpath="category_sequences.png",
    )
...
def table_multiple_categories():
    ...
    for i, metric in enumerate(metrics_new_names):
        ...
        yerr = np.stack(
            [
                my_df.groupby("category")[metric].min().to_numpy(),
                my_df.groupby("category")[metric].max().to_numpy(),
            ],
        )
...
def table_multiview_sequences():
    ...
    # metrics = ['test/co3d_5s_test/pose/acc_pi6', 'test/co3d_5s_test/pose/acc_pi18', 'test/co3d_5s_test/pose/err_median', 'test/co3d_5s_test/pose/err_mean']
    metrics = [
        "test/co3d_50s_test/pose/acc_pi6",
        "test/co3d_50s_test/pose/acc_pi18",
        "test/co3d_50s_test/pose/err_median",
        "test/co3d_50s_test/pose/err_mean",
    ]
    name_partial = "multiview"
    configs = [
        "method.value.multiview.type",
        "method.value.multiview.batch_size",
        "method.value.inference.refine.dims_detached",
    ]
    age_in_hours = 6
    ...
    my_df.loc[
        my_df["method.value.inference.refine.dims_detached"].map(len) == 0,
        "method.value.multiview.type",
    ] = "multiview+translation"
    my_df["seqs"] = (
        my_df["Run"].str.split("s_").str[0:2].str.join("_").str[-10:].replace(map_seqs)
    )
    ...
    my_df = my_df.rename(
        columns={
            "seqs": "train seqs.",
            "method.value.multiview.type": "inference type",
            "method.value.multiview.batch_size": "multiview #frames",
            metrics[0]: metrics_new_names[0],
            metrics[1]: metrics_new_names[1],
            metrics[2]: metrics_new_names[2],
            metrics[3]: metrics_new_names[3],
...
def table_multiview():
    ...
    my_df = my_df.groupby(
        ["method.value.multiview.type", "method.value.multiview.batch_size"],
    ).head(1)
    my_df = my_df.rename(
        columns={
            "method.value.multiview.type": "type",
            "method.value.multiview.batch_size": "multiview #frames",
            metrics[0]: metrics_new_names[0],
            metrics[1]: metrics_new_names[1],
            metrics[2]: metrics_new_names[2],
...
def get_run_name_regex(
    ...
    run_name_regex = get_run_name(
        bench_name=bench_regex,
        method_name=method_regex,
        platform_name=platform_regex,
        ablation_name=ablations_regex,
...
def get_ablations_fpaths(ablation: str = None):
    ...
    for ablation_fpaths_rel in ablations_fpaths_rel:
        ...
        for ablation_fpath_rel in ablation_fpaths_rel:
            ablations_fpaths[-1].append(
                ablations_root_dir.joinpath(f"{ablation_fpath_rel}.yaml"),
            )
        ...
        for ablation_dir in ablation_dirs:
            ...
            for ablation_file_fpath in ablation_dir.iterdir():
                if ablation_file_fpath.is_dir():
                    continue
                ablation_fpath_rel = ablation_file_fpath.relative_to(
                    ablations_root_dir,
                ).with_suffix("")
...
def get_ablations_configs(ablation: str = None):
    ...
    for ablation_configs_fpaths in ablations_configs_fpaths:
        for fpath in ablation_configs_fpaths:
            config = dict(
                read_yaml(fpath, resolve=False),
            )  # problem: ablation not interpolatable
...
def get_ablations_regex(ablation: str = None):
    ...
    ablation_regex = ")_(".join(
        [
            "|".join(
                [ablation_fpath_rel.stem for ablation_fpath_rel in ablation_fpaths_rel],
            )
            for ablation_fpaths_rel in ablations_fpaths_rel
        ],
    )
...
def get_ablations_fpaths_rel_comb(ablation: str = None):
    ...
    logger.info(
        f"loading {len(combinations_ablation_fpaths_rel)} hierarchical ablations...",
    )
...
def multiple(
    ...
    if ablation is None:
        cfgs = [
            od3d.io.load_hierarchical_config(benchmark=benchmark, platform=platform),
        ]
    else:
        combinations_ablations_fpaths_rel = get_ablations_fpaths_rel_comb(
            ablation=ablation,
        )
        logger.info(
            f"loading {len(combinations_ablations_fpaths_rel)} hierarchical ablations...",
        )
        cfgs = od3d.io.load_multiple_hierarchical_configs(
            benchmark=benchmark,
            platform=platform,
    ...
    if ablation is not None:
        prev_runs_lt = get_runs_multiple(
            benchmark=benchmark,
            ablation=ablation,
            age_in_hours_lt=age_in_hours_lt,
        ...
        prev_runs_names_without_ts_and_platform = [
            get_run_name_without_ts_and_platform(run.name) for run in prev_runs_lt
        ]
    else:
        prev_runs_names_without_ts_and_platform = []
    ...
    prev_runs_all_names_without_ts_and_platform = [
        get_run_name_without_ts_and_platform(run_name)
        for run_name in prev_runs_all_names
    ]
    prev_runs_all_names = prev_runs_all_names[::-1]
    prev_runs_all_names_without_ts_and_platform = (
        prev_runs_all_names_without_ts_and_platform[::-1]
    )  # reverse to order from latest to oldest
    ...
    for i, method_cfg in tqdm(enumerate(methods_cfgs)):
        ...
        with open_dict(method_cfg):
            ...
            run_name = get_run_name(
                bench_name=method_cfg.train_datasets.labeled.class_name,
                method_name=method_cfg.method.class_name,
                platform_name=method_cfg.platform.link,
                ablation_name=ablation_name,
            )
            run_name_without_ts_and_platform = get_run_name_without_ts_and_platform(
                run_name,
            )
            if (
                run_name_without_ts_and_platform
                in prev_runs_names_without_ts_and_platform
            ):
            ...
            if checkpoint is not None and "__LAST_RUN__" in checkpoint:
                if (
                    run_name_without_ts_and_platform
                    in prev_runs_all_names_without_ts_and_platform
                ):
                    method_cfg.method.checkpoint = method_cfg.method.checkpoint.replace(
                        "__LAST_RUN__",
                        prev_runs_all_names[
                            prev_runs_all_names_without_ts_and_platform.index(
                                run_name_without_ts_and_platform,
                            )
                        ],
                    )
                    logger.info(
                        f"Found last checkpoint for {run_name} in {method_cfg.method.checkpoint}",
                    )
                else:
                    logger.warning(
                        f"Could not find last checkpoint for {run_name}. Running anyway...",
                    )
...
def get_failed_runs(name_regex=".*", age_in_hours=1000):
    ...
    runs = list(
        filter(lambda run: run.state == "failed" or run.state == "crashed", runs),
...
@app.command()
def delete_wandb(
    age_in_hours_gt: int = typer.Option(0, "-g", "--greater"),
...
@app.command()
def delete_wandb_failed(
    age_in_hours_gt: int = typer.Option(0, "-g", "--greater"),
    age_in_hours_lt: int = typer.Option(1000, "-l", "--lower"),
    name_regex: str = typer.Option(".*", "-n", "--name"),
):
    logging.basicConfig(level=logging.INFO)
    runs_failed = get_runs(
        name_regex=name_regex,
        age_in_hours_lt=age_in_hours_lt,
        age_in_hours_gt=age_in_hours_gt,
...
def restart_slurm(
    ...
    for run_name in runs_names:
        # cfg.path_home
        local_tmp_config_fpath = Path(cfg.path_home).joinpath(
            "tmp",
            f"config_{run_name}.yaml",
        )
        try:
...
@app.command()
def test(
    benchmark: str = typer.Option("timeseries_internal", "-b", "--benchmark"),
...
@app.command()
def rsync(
    platform_source: str = typer.Option("slurm", "-s", "--source"),
    ...
    od3d.io.run_cmd(
        cmd=f"rsync -avrzP {source_link}{path_source} {target_link}{path_target.parent}",
        live=True,
        logger=logger,
    )
