...
def error_choices(
    ...
    results = OD3D_Results.read_from_local(
        logging_dir=run_path,
        dataset_rpath=Path(dataset_rpath),
    )
...
class SurveyApp:
    ...
    def create_widgets(self):
        ...
        for i, choice in enumerate(self.choices):
            self.choices_checkbuttons[choice] = ttk.Checkbutton(
                self.master,
                text=choice,
                variable=self.choices_var[choice],
            )
            self.choices_checkbuttons[choice].grid(
                row=i + 1,
                column=0,
                padx=10,
                pady=5,
    ...
    def show_results(self):
        ...
        for choice in self.choices_selected:
            logger.info(
                f"{choice}: {100 * sum(self.choices_selected[choice]) / len(self.choices_selected[choice])} %",
            )
