...
class NeMoBackbone(Extractor):
    ...
    def __init__(self, config: DictConfig):
        super(NeMoBackbone, self).__init__(config)  # Update with default configuration.
    ...
    def load_checkpoint(self, checkpoint_path: str = None):
        if checkpoint_path is not None:
            ...
            self.net.load_state_dict(state_dict, strict=self.config.get("strict_loading", False))
        elif self.config.get("checkpoint", None) is not None:
            state_dict = torch.load(self.config.checkpoint, map_location=self.device)
            if "state_dict" in state_dict:
                state_dict = state_dict["state_dict"]
            assert "strict_loading" in self.config, "If checkpoint is given, strict_loading must be specified"
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        assert all([kw in data for kw in self.required_keywords]), f"Required keywords: {self.required_keywords}. Found keywords: {list(data.keys())}."
        ...
        features_kp = nn.functional.grid_sample(
            features, keypoints.unsqueeze(1)
    ...
    def forward2(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        assert all([kw in data for kw in self.required_keywords]), f"Required keywords {self.required_keywords} not found in data."
        image = data["image"]  # (B, C, H, W)
        features = self.net(image)  # (B, C, H//R, W//R)
        ...
        # Extract features at keypoints.
        features_kp = nn.functional.grid_sample(features, keypoints.unsqueeze(1))  # check that this works
        result = {
            "keypoints": keypoints,
