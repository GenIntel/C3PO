...
def co_pca(features1, dim=[128,128,128]):
    ...
    # Compute the PCA
    for name, tensors in zip(['s5', 's4', 's3'], [[s5_1, s5_1], [s4_1, s4_1], [s3_1, s3_1]]):
        target_dim = target_dims[name]
...
if __name__ == '__main__':
    ...
    img_1 = np.array(Image.open('/CT/3D_DST_Scene/work/od3d/datasets/CO3D_V2_Preprocess/sfm/colmap50/hydrant/167_18177_33907/Partial_Ratio_25.0_Percent_start_frame_0_flip_sfm_False/images/frame000003.jpg').resize((960,960), Image.ANTIALIAS))
    img_2 = np.array(Image.open('/CT/3D_DST_Scene/work/od3d/datasets/CO3D_V2_Preprocess/sfm/colmap50/hydrant/167_18177_33907/Partial_Ratio_25.0_Percent_start_frame_0_flip_sfm_False/images/frame000003.jpg').resize((960,960), Image.ANTIALIAS))
    ...
    features1 = process_features_and_mask(sd_model, sd_aug, img_1, input_text=None,  mask=False, raw=True)
    ...
    normalized_image_1 = (reduced_image_1 - reduced_image_1.min()) / (reduced_image_1.max() - reduced_image_1.min()) 
    
    features2 = process_features_and_mask(sd_model, sd_aug, img_2, input_text=None,  mask=False, raw=True)
    ...
    mask_1 = np.array(Image.open('/CT/CO3D-dataset/nobackup/CO3D_V2/hydrant/167_18177_33907/masks/frame000003.png').resize((int(W1/8),int(H1/8))))
    mask_2 = np.array(Image.open('/CT/CO3D-dataset/nobackup/CO3D_V2/hydrant/167_18177_33907/masks/frame000003.png').resize((int(W1/8),int(H1/8))))
    
    mask_1[mask_1 > 0] =1
    mask_2[mask_2 > 0] =1

    features1_resized = torch.nn.functional.interpolate(features1, size=(int(H1/8),int(W1/8)), mode='bilinear')
    features2_resized = torch.nn.functional.interpolate(features2, size=(int(H1/8),int(W1/8)), mode='bilinear')
    ...
    matches_im0, matches_im1 = fast_reciprocal_NNs(features1_resized, features2_resized, subsample_or_initxy1=8,device= 'cuda', dist='dot', block_size=2**13)
    plt.figure()
    
    img_1 = np.array(Image.open('/CT/3D_DST_Scene/work/od3d/datasets/CO3D_V2_Preprocess/sfm/colmap50/hydrant/167_18177_33907/Partial_Ratio_25.0_Percent_start_frame_0_flip_sfm_False/images/frame000003.jpg').resize((int(W1/8),int(H1/8)), Image.ANTIALIAS))
    img_2 = np.array(Image.open('/CT/3D_DST_Scene/work/od3d/datasets/CO3D_V2_Preprocess/sfm/colmap50/hydrant/167_18177_33907/Partial_Ratio_25.0_Percent_start_frame_0_flip_sfm_False/images/frame000003.jpg').resize((int(W1/8),int(H1/8)), Image.ANTIALIAS))
