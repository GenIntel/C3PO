...
class ResNet(OD3D_Backbone):
    def __init__(
        ...
        )  # choose from [1, 2, 3, 4], start with depeest layer (4)
        self.layers_count = len(self.layers_returned)

        resnet = torchvision.models.resnet50(
            weights=MAP_RESNET50_WEIGHTS[config.weights],
        )
        ...
        self.out_dims = [
            self.layers[layer_id - 1][-1].conv3.out_channels
            for layer_id in self.layers_returned
        ]
        self.out_downsample_scales = [
            2 ** (self.layers_returned[i] - self.layers_returned[i + 1])
