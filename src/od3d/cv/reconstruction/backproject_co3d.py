...
def visualize_tensor(feature_tensor, name):
    ...
    reshaped_features = feature_tensor.reshape(768, -1).T  # Transpose to get [1024, 768]
    
    # PCA: Reduce dimensions to 3 for RGB visualization
    pca = PCA(n_components=3)
    reduced_features = pca.fit_transform(reshaped_features)
    # Normalize for RGB
    colors = (reduced_features - np.min(reduced_features, axis=0)) / (np.max(reduced_features, axis=0) - np.min(reduced_features, axis=0))
...
def backproject_with_feat(depth, feat, intrinsics, cam2world, mesh_vertices, meshes_verts_aggregated_features_test):
    height, width = depth.shape
    ...
    feat_resized = torch.nn.functional.interpolate(feat.unsqueeze(0), (height, width),mode='nearest' )
    ...
    if len(points_3d_flatten)!=0:
        ...
        indices = np.random.choice(feated_points_world.shape[0], num_samples, replace=False)
...
def tell_left_from_right_sd_feature(ref_sd_feature, src_sd_feature, src_sd_feature_flipped, mask_ref, mask_src, mask_src_flipped, device = 'cuda'):
    matches_im0, matches_im1 = fast_reciprocal_NNs(ref_sd_feature, src_sd_feature, subsample_or_initxy1=8, device=device, dist='dot', block_size=2**13)
