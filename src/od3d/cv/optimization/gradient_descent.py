...
def gradient_descent_se3(pts, models, score_func, pts_dist=None, steps=10, beta0=0.4, beta1=0.6, lr=2e-5, pts_weight=0.5, arap_weight=0.05, arap_geo_std=0.02, dims_detached=[], return_pts_offset=False):
    """
    Args:
    ...
    for s in range(steps):
        ...
        pairwise_dist = torch.cdist(pts, pts, p=2)
        ...
        scores_arap = -(weights_arap * ((pairwise_dist - torch.cdist(pts + pts_weight * pts_offset, pts + pts_weight * pts_offset, p=2)).abs() / pairwise_dist.max())).flatten(-2).mean(dim=-1)
        ...
        loss = (-scores).sum() + arap_weight * (-scores_arap).sum()
        loss.backward()
