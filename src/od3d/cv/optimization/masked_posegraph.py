...
def random_pose3():
    ...
    return gtsam.Pose3(gtsam.Rot3.Rodrigues(-0.1 + 0.2 * np.random.rand(),
                                             -0.1 + 0.2 * np.random.rand(),
                                             -0.1 + 0.2 * np.random.rand()),
                       gtsam.Point3(-0.5 + np.random.rand(), 
                                    -0.5 + np.random.rand(), 
                                    -0.5 + np.random.rand()))
...
class WeightedPoseGraphOptimizationRotationMatrixMasking():
    def __init__(self, rotation_matrices, weighted_matrices, seq_list = None) -> None:
        ...
        self.gt_ref_tform_src = torch.eye(4)  # Keeping 4x4 for comparison with a generic identity matrix
    ...
    def random_rotation_se3(self):
        random_rotation = gtsam.Rot3.Rodrigues(np.random.randn(), np.random.randn(), np.random.randn())
    ...
    def optimization(self):
        for i in range(self.n):
            for j in range(self.n):
                if i != j:
                    Rij = self.convert_rotation_to_gtsam_pose3(self.rotation_matrices[i][j])
                    weight = self.weight_matrices[i][j].cpu().numpy()
                    self.graph.add(gtsam.BetweenFactorPose3(self.symbols[i], self.symbols[j], 
                                                            Rij, gtsam.noiseModel.Diagonal.Variances(np.ones(6) * (1/weight))))
        initial_estimate = gtsam.Values()
        initializer = PGOInitializer(self.rotation_matrices, self.weight_matrices, self.seq_list)
        ...
        optimizer = gtsam.LevenbergMarquardtOptimizer(self.masked_graph, initial_estimate)
    ...
    def return_pose_node_optimized(self):
        ...
        for i in range(self.n):
            ...
            if torch.cuda.is_available():
                rot_matrix_tensor = rot_matrix_tensor.cuda()  # Ensuring the tensor is on GPU if available
...
class PGOInitializer:
    def __init__(self, transformation_matrices, weight_matrix, seq_list):
        ...
        self.selected = []
        self.remaining = list(range(transformation_matrices.shape[0]))  # Assuming transformation_matrices is a numpy array
        self.pose_node = {}
        self.idx = 0
        self.node_order = []
        self.edges = []
    ...
    def start(self):
        ...
        if max_pos is not None:
            ...
            self.pose_node[str(max_pos[1])] =  torch.linalg.inv(self.transformation_matrices[max_pos[0], max_pos[1]])
            ...
            self.edges.append(
                {
                    ...
                    'transformation':self.transformation_matrices[max_pos[0], max_pos[1]].tolist()
                }
            )
    ...
    def _iterate(self):
        ...
        while self.remaining:
            ...
            if added_new_node is not None:
                ...
                inverse_transform = torch.linalg.inv(self.transformation_matrices[used_node, added_new_node])
                self.pose_node[str(added_new_node)] = inverse_transform @ self.pose_node[str(used_node)].cuda()
                ...
                self.edges.append(
                {
                    ...
                    'transformation':self.transformation_matrices[used_node, added_new_node].tolist()
                }
                )
                self.edge_nodes.append([used_node, added_new_node])
                self.weights.append(float(self.weight_matrix[used_node, added_new_node]))
        return self.pose_node
