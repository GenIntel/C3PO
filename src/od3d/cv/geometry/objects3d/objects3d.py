...
class OD3D_Objects3D(abc.ABC, nn.Module):
    ...
    def __init__(self, feat_dim=128, objects_count=0, feat_clutter=False, feats_objects=False, feats_requires_grad=True,
                 feats_distribution=FEATS_DISTR.VON_MISES_FISHER, device=None, dtype=None):

        factory_kwargs = {'device': device, 'dtype': dtype}

        super(OD3D_Objects3D, self).__init__()
        self.feat_dim = feat_dim
        self.objects_count = objects_count

        if feat_clutter:
            self.feat_clutter = torch.nn.Parameter(torch.empty(self.feat_dim, **factory_kwargs),
                                                   requires_grad=feats_requires_grad)  # F,
        else:
    ...
    def normalize_feats(self):
        if self.feat_clutter is not None:
            self.feat_clutter.data = (
                    ...
                    / self.feat_clutter.detach().norm(dim=-1, keepdim=True)
            )

    def __len__(self):
        return self.objects_count

    def cams_downsample(self, cams_intr4x4=None, imgs_sizes=None, down_sample_rate=1.):
        """
    ...
    def cams_and_objects_broadcast(self, cams_tform4x4_obj, cams_intr4x4, objects_ids):
        ...
        objects_ids = (
            objects_ids[:, None].expand(objects_count, cams_count).reshape(-1)
        )
    ...
    def render(self, cams_tform4x4_obj, cams_intr4x4, imgs_sizes, objects_ids=None,
               modalities: Union[PROJECT_MODALITIES, List[PROJECT_MODALITIES]]=PROJECT_MODALITIES.FEATS,
               broadcast_batch_and_cams=False, down_sample_rate=1.0,
        ...
        cams_intr4x4, imgs_sizes = self.cams_downsample(cams_intr4x4, imgs_sizes, down_sample_rate)
        ...
        if broadcast_batch_and_cams:
            ...
            cams_tform4x4_obj, cams_intr4x4, objects_ids = self.cams_and_objects_broadcast(
                cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4, objects_ids=objects_ids)
        ...
        mods2d_rendered = self.render_batch(cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4,
                                            imgs_sizes=imgs_sizes, objects_ids=objects_ids,
                                            modalities=_modalities, add_clutter=add_clutter,
    ...
    def sample_with_img2d(self, img2d=None, img2d_mask=None, cams_tform4x4_obj=None, cams_intr4x4=None, imgs_sizes=None,
                          objects_ids=None,
                          modalities: Union[PROJECT_MODALITIES, List[PROJECT_MODALITIES]]=PROJECT_MODALITIES.FEATS,
                          broadcast_batch_and_cams=False, down_sample_rate=1.0,
                          add_clutter=False, add_other_objects=False, dtype=None, device=None,
                          sample_clutter_count=0, clutter_pxl2d=None):
        """
        ...
        mods1d_sampled = self.sample(
            ...
            cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4, imgs_sizes=imgs_sizes,
            objects_ids=objects_ids, broadcast_batch_and_cams=broadcast_batch_and_cams,
            down_sample_rate=down_sample_rate, add_clutter=add_clutter, add_other_objects=add_other_objects,
            dtype=dtype, device=device)

        feats1d_obj_mask, feats1d_obj_pxl2d = mods1d_sampled[PROJECT_MODALITIES.MASK], mods1d_sampled[PROJECT_MODALITIES.PXL2D]

        if sample_clutter_count > 0:
            feats2d_obj_mask = self.render(
                modalities=PROJECT_MODALITIES.MASK, cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4,
                imgs_sizes=imgs_sizes, objects_ids=objects_ids, broadcast_batch_and_cams=broadcast_batch_and_cams,
                down_sample_rate=down_sample_rate, add_clutter=False, add_other_objects=False)
        ...
        if clutter_pxl2d is not None:
            ...
            feats1d_clutter_mask = torch.ones((B, sample_clutter_count), dtype=torch.bool, device=feats1d_obj_mask.device)
            feats1d_obj_mask = torch.cat([feats1d_obj_mask, feats1d_clutter_mask], dim=-1)
        ...
        for modality in _modalities:
            ...
            if modality == PROJECT_MODALITIES.IMG:
                ...
                label1d_clutter = self.get_clutter_label(add_other_objects=add_other_objects, one_hot=True,
                                                         device=device)[None, :, None, ].expand(B, V,
                                                                                                sample_clutter_count)
                mods1d_sampled[modality] = torch.cat([mods1d_sampled[modality], label1d_clutter], dim=-1)
            elif modality == PROJECT_MODALITIES.ID:
                B = mods1d_sampled[modality].shape[0]
                label1d_clutter = self.get_clutter_label(add_other_objects=add_other_objects, one_hot=False, device=device)[
                    None,].expand(B, sample_clutter_count)
                mods1d_sampled[modality] = torch.cat([mods1d_sampled[modality], label1d_clutter], dim=-1)
    ...
    def sample(self, cams_tform4x4_obj=None, cams_intr4x4=None, imgs_sizes=None, objects_ids=None,
               modalities: Union[PROJECT_MODALITIES, List[PROJECT_MODALITIES]]=PROJECT_MODALITIES.FEATS,
               broadcast_batch_and_cams=False, down_sample_rate=1.0,
               add_clutter=False, add_other_objects=False, dtype=None, device=None,
        ...
        mods1d_rendered = self.sample_batch(cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4,
                                            imgs_sizes=imgs_sizes, objects_ids=objects_ids,
                                            modalities=_modalities, add_clutter=add_clutter,
                                            add_other_objects=add_other_objects, device=device, dtype=dtype,
                                            sample_other_objects=sample_other_objects, sample_clutter=sample_clutter)
    ...
    def get_sim_render(self, feats2d_img, cams_tform4x4_obj, cams_intr4x4, objects_ids=None,
                       broadcast_batch_and_cams=False, down_sample_rate=1.0,
                       feats2d_img_mask=None, allow_clutter=True, return_sim_pxl=False,
                       add_clutter=False, add_other_objects=False, temp=1.0):
        """
        Args:
        ...
        feats2d_rendered = self.render(modalities=PROJECT_MODALITIES.FEATS, cams_tform4x4_obj=cams_tform4x4_obj,
                                       cams_intr4x4=cams_intr4x4, imgs_sizes=imgs_sizes, objects_ids=objects_ids,
                                       broadcast_batch_and_cams=broadcast_batch_and_cams,
                                       down_sample_rate=down_sample_rate, add_clutter=add_clutter,
                                       add_other_objects=add_other_objects)

        return self.get_sim_feats2d_img_and_rendered(feats2d_img, feats2d_rendered,
                                                     return_sim_pxl=return_sim_pxl,
                                                     feats2d_img_mask=feats2d_img_mask,
                                                     allow_clutter=allow_clutter, temp=temp)



    def sample_nearest_to_feats2d_img(
            self, feats2d_img, cams_tform4x4_obj=None, cams_intr4x4=None, imgs_sizes=None, objects_ids=None,
            modalities: Union[PROJECT_MODALITIES, List[PROJECT_MODALITIES]]=PROJECT_MODALITIES.FEATS,
            broadcast_batch_and_cams=False, down_sample_rate=1.0, add_clutter=False, add_other_objects=False,
            dtype=None, device=None,
            smooth_labels=False, sim_temp=1.0,):
        """
        Args:
        ...
        sim_feats2d = self.get_sim_feats2d_img_to_all(
            feats2d_img=feats2d_img, imgs_sizes=imgs_sizes, cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4,
            objects_ids=objects_ids, broadcast_batch_and_cams=broadcast_batch_and_cams,
            down_sample_rate=down_sample_rate, add_clutter=add_clutter, add_other_objects=add_other_objects,
            dense=True, sim_temp=sim_temp, clutter_pxl2d=None)

        label_feats2d_nearest = sim_feats2d.argmax(dim=1, keepdim=True) # (B, V(+1), V+N) or (B, V(+1), H, W) if dense=True

        nearest_mods2d = self.sample(
            modalities=modalities, cams_tform4x4_obj=cams_tform4x4_obj,
            cams_intr4x4=cams_intr4x4, imgs_sizes=imgs_sizes, objects_ids=objects_ids,
            broadcast_batch_and_cams=broadcast_batch_and_cams, down_sample_rate=down_sample_rate,
            add_clutter=add_clutter, add_other_objects=add_other_objects, sample_clutter=add_clutter,
            sample_other_objects=add_other_objects) # BxV(+1)x3

        from od3d.cv.select import batched_index_select
        B, _, H, W = label_feats2d_nearest.shape
        nearest_mods2d = batched_index_select(input=nearest_mods2d, index=label_feats2d_nearest.flatten(1), dim=1).permute(0, 2, 1).reshape(B, 3, H, W) # Bx3xHxW
        return nearest_mods2d

    def get_sim_feats2d_img_to_all(self, feats2d_img, imgs_sizes, cams_tform4x4_obj=None, cams_intr4x4=None, objects_ids=None,
                                   broadcast_batch_and_cams=False, down_sample_rate=1.0,
                                   add_clutter=True, add_other_objects=True, dense=False,
                                   sim_temp=1.0, clutter_pxl2d=None, return_feats=False):
        """
        Args:
            feats2d_img (torch.Tensor): BxFxHxW
            feats1d_obj (torch.Tensor): BxVxF
        ...
        if dense:
            feats1d_sampled = self.sample(
                modalities=PROJECT_MODALITIES.FEATS,  cams_tform4x4_obj=cams_tform4x4_obj,
                cams_intr4x4=cams_intr4x4, imgs_sizes=imgs_sizes, objects_ids=objects_ids,
                broadcast_batch_and_cams=broadcast_batch_and_cams, down_sample_rate=down_sample_rate,
                add_clutter=add_clutter, add_other_objects=add_other_objects)
            sim_feats2d = self.get_sim_feats2d_img_and_feats1d_obj(feats2d_img, feats1d_sampled, add_clutter=False,
                                                                   temp=sim_temp)
            if return_feats:
            ...
            mods1d_sampled = self.sample_with_img2d(img2d=feats2d_img,
                modalities=[feat_modality, PROJECT_MODALITIES.IMG],
                cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4, imgs_sizes=imgs_sizes,
                objects_ids=objects_ids, broadcast_batch_and_cams=broadcast_batch_and_cams,
                down_sample_rate=down_sample_rate, add_clutter=add_clutter, add_other_objects=add_other_objects,
                                                    clutter_pxl2d=clutter_pxl2d,
                dtype=feats2d_img.dtype, device=feats2d_img.device)


            sim_feats1d = self.get_sim_feats1d_img_and_feats1d_obj(mods1d_sampled[PROJECT_MODALITIES.IMG],
                                                                   mods1d_sampled[feat_modality], add_clutter=False,
                                                                   temp=sim_temp)
    ...
    def get_label_feats2d_img(self, feats2d_img, imgs_sizes, cams_tform4x4_obj, cams_intr4x4, objects_ids=None,
                              broadcast_batch_and_cams=False, down_sample_rate=1.0, feats2d_img_mask=None,
                              add_clutter=True, add_other_objects=True, sample_clutter_count=5, dense=False,
                              smooth_labels=False):
        """
        Args:
            feats2d_img (torch.Tensor): BxFxHxW
        ...
        if dense:
            ...
            label2d = self.render(modalities=label_modality, cams_tform4x4_obj=cams_tform4x4_obj,
                                  cams_intr4x4=cams_intr4x4, imgs_sizes=imgs_sizes,
                                  objects_ids=objects_ids, broadcast_batch_and_cams=broadcast_batch_and_cams,
                                  down_sample_rate=down_sample_rate, add_clutter=add_clutter,
                                  add_other_objects=add_other_objects)
            ...
            mods1d_sampled = self.sample_with_img2d(img2d=feats2d_img, img2d_mask=feats2d_img_mask,
                modalities=[mask_modality, label_modality, PROJECT_MODALITIES.CLUTTER_PXL2D],
                cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4, imgs_sizes=imgs_sizes,
                objects_ids=objects_ids, broadcast_batch_and_cams=broadcast_batch_and_cams,
                down_sample_rate=down_sample_rate, add_clutter=add_clutter, add_other_objects=add_other_objects,
                sample_clutter_count=sample_clutter_count)

            return mods1d_sampled[label_modality], mods1d_sampled[mask_modality], mods1d_sampled[PROJECT_MODALITIES.CLUTTER_PXL2D]

    def get_clutter_label(self, add_other_objects=False, one_hot=False, device=None):
        raise NotImplementedError

    def get_label_and_sim_feats2d_img_to_all(self, feats2d_img, imgs_sizes, cams_tform4x4_obj, cams_intr4x4, objects_ids=None,
                                             broadcast_batch_and_cams=False, down_sample_rate=1.0, feats2d_img_mask=None,
                                             add_clutter=True, add_other_objects=True, sample_clutter_count=5, dense=False,
                                             smooth_labels=False, sim_temp=1.0, return_feats=False):
        """
        Args:
            feats2d_img (torch.Tensor): BxFxHxW
            feats1d_obj (torch.Tensor): BxVxF
        ...
        label_feats, label_feats_mask, feat_clutter_pxl2d = self.get_label_feats2d_img(
            feats2d_img=feats2d_img, imgs_sizes=imgs_sizes, cams_tform4x4_obj=cams_tform4x4_obj, cams_intr4x4=cams_intr4x4,
            objects_ids=objects_ids, broadcast_batch_and_cams=broadcast_batch_and_cams,
            down_sample_rate=down_sample_rate, feats2d_img_mask=feats2d_img_mask, add_clutter=add_clutter,
            add_other_objects=add_other_objects, sample_clutter_count=sample_clutter_count, dense=dense,
        ...
        sim_feats = self.get_sim_feats2d_img_to_all(
            ...
            objects_ids=objects_ids, broadcast_batch_and_cams=broadcast_batch_and_cams,
            down_sample_rate=down_sample_rate, add_clutter=add_clutter, add_other_objects=add_other_objects,
            dense=dense, sim_temp=sim_temp, clutter_pxl2d=feat_clutter_pxl2d, return_feats=return_feats)

        if return_feats:
            return label_feats, label_feats_mask, feat_clutter_pxl2d, sim_feats[0], sim_feats[1]
        else:
    ...
    def get_sim_cams_verts(self, categories_ids, feats2d_net, verts2d_mesh, verts2d_mesh_mask, return_sim_pxl=False,
    ...
    def get_sim_feats1d_img_and_feats1d_obj(self, feats1d_img, feats1d_obj, add_clutter=False, temp=1.0):
        """
        Args:
    ...
    def get_sim_feats2d_img_and_feats1d_obj(self, feats2d_img, feats1d_obj, add_clutter=False, temp=1.0):
        """
        Args:
            feats2d_img (torch.Tensor): BxCxHxW
            feats1d_obj (torch.Tensor): BxVxC
    ...
    def get_sim_feats2d_img_and_rendered(self, feats2d_img, feats2d_rendered, return_sim_pxl=False,
                                         feats2d_img_mask=None, allow_clutter=True, temp=1.0):
        """
        Args:
            feats2d_img (torch.Tensor): BxCxHxW
        ...
        # shape (B, V, H, W)
        sim_clutter2d = self.get_sim("bchw,c->bhw", feats2d_img, self.feat_clutter,
                                     temp=temp)[:, None].expand(sim_feats2d.shape)
