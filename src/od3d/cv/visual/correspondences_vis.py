...
from od3d.methods.nemo_align3d.utils import get_max_cosine_similarity_vertex_pairs, get_min_cosine_similarity_vertex_pairs
import time
import open3d as o3d
import shutil
# THRESHOLD = 0.90
...
def plot(angle_list, max_value_list_mean_list, min_value_list_mean_list):
    ...
    plt.scatter(max_values, angle_array, color='red', alpha=0.5, label='Max Value vs. Angle')
    ...
    plt.scatter(min_values, angle_array, color='blue', alpha=0.5, label='Min Value vs. Angle')
...
def compare_sph_dino(models, pts_src_ids, pts_ref_ids, sph_src, sph_ref, scores, r, s, category,sph_src_filtered, sph_ref_filtered):
    ...
    for i in range(len(models)):
        ...
        for j in range(4):
            ...
            trace_variance_src, average_variance_src, largest_variance_src = cal_var(feat_src)
            trace_variance_ref, average_variance_ref, largest_variance_ref = cal_var(feat_ref)
            ...
            max_value = get_max_cosine_similarity_vertex_pairs(sph_src[pts_src_id[j]], sph_ref[pts_ref_id[j]])
            min_value = get_min_cosine_similarity_vertex_pairs(sph_src[pts_src_id[j]], sph_ref[pts_ref_id[j]])
        ...
        count_low_var = sum(1 for var_src,var_ref in  zip(var_src_list,var_ref_list)  if var_src < VAR_MAX and var_ref < VAR_MAX  )
    ...
    plt.scatter(score_list, diff_rot_angle_rad_list, color='red', alpha=0.5, label='Max Value vs. Angle')
    plt.title('Score Value vs. Angle')
    plt.ylabel('Angle Difference (radians)')
    plt.xlabel('Scores')
    plt.legend()
    os.makedirs(os.path.join('test', f'cate_{category}_theshold_{THRESHOLD}'), exist_ok=True)
    plt.savefig(os.path.join('test',  f'cate_{category}_theshold_{THRESHOLD}',f'scores_angles_ref_name_{r}_src_name_{s}.png'))
    if len(score_list) > 0:
        my_best_score_id = np.argmax(score_list)
        print('MY BEST diff_rot_angle_rad_list is ', diff_rot_angle_rad_list[my_best_score_id] * 180 / np.pi )
...
def compare_sph_dino_best_id(model, pts_src_id, pts_ref_id, sph_src, sph_ref, pts_ref, pts_src, sph_src_filtered, sph_ref_filtered):
    gt_ref_tform_src = torch.eye(4).cuda()
    ...
    for id in range(4):
        max_value = get_max_cosine_similarity_vertex_pairs(sph_src[pts_src_id[id]], sph_ref[pts_ref_id[id]])
        min_value = get_min_cosine_similarity_vertex_pairs(sph_src[pts_src_id[id]], sph_ref[pts_ref_id[id]])
...
def plot_relationships(dist_list, feat_dist_list, angle_list, scores, geo_score, semantic_score):
    ...
    plt.subplot(1, 5, 2)
    plt.scatter(feat_dist_array, angle_array, c='b', alpha=0.5, label='Feat Dist vs. Angle')
    ...
    plt.subplot(1, 5, 5)
    plt.scatter(semantic_score, angle_array, c='g', alpha=0.5, label='Geo Scores vs. Angle')
...
def get_closest_correspondence_model(models, best_correspondence, best_ref_correspondence, pts, pts_ref, dist_ref_src, scores, geo_score, semantic_score):
    dist_list = []
    feat_dist_list = []
    angle_list = []
    for i in range(len(models)):
        ...
        feat_dist_sum = 0
        for j in range(4):
            feat_dist = dist_ref_src[best_correspondence_id[j]][best_ref_correspondence_id[j]]
    ...
    plot_relationships(dist_list, feat_dist_list, angle_list, scores, geo_score, semantic_score)
    # feat_dist_list = torch.tensor(feat_dist_list)
...
def vis_heatmap_from_vertices(dist_ref_src, pts_ref, pts_src, pts_ref_ids, pts_ids, filename):
...
def save_visualization_mesh_with_color(pts, # ref
        pts_ref, # src
        pts_ids, 
        pts_ref_ids, 
        filename, 
    ...
    # Create point clouds
    pts_point_cloud = o3d.geometry.PointCloud()
    pts_point_cloud.points = o3d.utility.Vector3dVector(original_pts_ref.clone().cpu().numpy())
    # pts_point_cloud.colors = o3d.utility.Vector3dVector(np.tile([0.3, 0.3, 0.3], (pts.shape[0], 1)))  # Color each point as gray
    pts_point_cloud.colors = o3d.utility.Vector3dVector( original_pts_color_ref.clone().cpu().numpy())

    pts_ref_point_cloud = o3d.geometry.PointCloud()
    pts_ref_point_cloud.points = o3d.utility.Vector3dVector(original_pts_src.clone().cpu().numpy())
    # pts_ref_point_cloud.colors = o3d.utility.Vector3dVector(np.tile([0.8, 0.2, 0.5], (pts_ref.shape[0], 1)))  # Color each point as lighter gray
    pts_ref_point_cloud.colors = o3d.utility.Vector3dVector( original_pts_color_src.clone().cpu().numpy() )  # Color each point as lighter gray

    # Save the point clouds to .ply files
    # o3d.io.write_point_cloud(os.path.join(filename, 'src.ply'), pts_point_cloud)
    # o3d.io.write_point_cloud(os.path.join(filename, 'ref.ply'), pts_ref_point_cloud)
    o3d.io.write_point_cloud(os.path.join(filename, 'ref_colored.ply'), pts_point_cloud)
    o3d.io.write_point_cloud(os.path.join(filename, 'src_colored.ply'), pts_ref_point_cloud)
...
def print_geo_feature_diff(pts_ids, pts_ref_ids, src_geo_feature, ref_geo_feature):
    ...
    for i in range(len(pts_ids)):
        feature_diff = torch.norm(ref_geo_feature[:,pts_ids[i],:] - src_geo_feature[:, pts_ref_ids[i], :])
        print(f'feature diff for {i}', feature_diff)

def find_nearest_neighbor_geo_feature(pts, pts_ref, pts_ids, pts_ref_ids, src_geo_features, ref_geo_feature, filename):
    distances = compute_l2_distance(ref_geo_feature, src_geo_features)  # B, N1, N2
    for i in range(len(pts_ids)):
        values, indices = torch.topk(distances[:,pts_ids[i],:], 5, largest= False, sorted= True)
        for j in range(5):
            ...
            # Save the LineSet to a .ply file
            o3d.io.write_line_set(os.path.join(filename, f'ref_{i}_src_{j}.ply'), line_set)
...
def save_pgo_init_meshes(pgo_init, meshes, mesh_ids, saved_folder ):
    for i in range(len(pgo_init)):
        ...
        convert_pts_mesh(pts_source, os.path.join(iteration_folder, f'old_node_id_{source}.ply' ))
        convert_pts_mesh(pts_target, os.path.join(iteration_folder, f'new_node_id_{target}.ply' ))



def save_pgo_init_meshes_transformed(pose_node, pts_list, pts_color_list, saved_folder, name_list):
    ...
    for i, node in enumerate(pose_node.keys()):
        ...
        pts = pts_list[int(node)].cpu().numpy()  # Ensure points are on CPU and in numpy format
        pts_color = pts_color_list[int(node)].cpu().numpy()  # Ensure colors are on CPU and in numpy format
        ...
        file_path = os.path.join(iteration_folder, f'pose_node_{i}_name_{name}_id_{int(node)}.ply')
        o3d.io.write_point_cloud(file_path, pts_point_cloud)
...
class MeshVisualizer():
    def __init__(self, ref_mesh, src_mesh, ref_correspondence, src_correspondence, output_dir):
    ...
    def save(self):
        o3d.io.write_point_cloud(os.path.join(self.output_dir, 'ref_mesh.ply'), self.ref_mesh)
        o3d.io.write_point_cloud(os.path.join(self.output_dir, 'src_mesh.ply'), self.src_mesh)
        torch.save(self.ref_correspondence, os.path.join(self.output_dir, 'ref_correspondence.pt'))
        torch.save(self.src_correspondence, os.path.join(self.output_dir, 'src_correspondence.pt'))
