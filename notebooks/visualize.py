...
Type = 'tsne'
sequence_list = [0] # M_dinov2_vitb14_frozen_base_no_norm_T_centerzoom512_R_acc ,M_nemo_old_T_centerzoom512_R_acc
co3d = CO3Dv1.create_by_name('co3dv1_zsp_10s' , config={'mesh_feats_type' :'M_dinov2_vitb14_frozen_base_no_norm_T_centerzoom512_R_acc','mesh_feats' :{'enabled':True,'override': False}})#, config={'preprocess' : {'tform_obj': {'enabled': True}},'mesh_feats' :{'enabled':False},'mesh_feats_dist' :{'enabled':False}}) #, config={'categories': [category], 'aligned_name': aligned_name, 'sequences_count_max_per_category': sequences_count}) # co3d_no_zsp_20s_aligned #co3d_5s_no_zsp_labeled 'co3d_50s_no_zsp_aligned' 'co3dv1_10s_zsp_aligned' 'co3d_10s_zsp_aligned' 'co3dv1_10s_zsp_unlabeled'
...
for i, idx in enumerate(sequence_list):
    ...
    seq_colors_padded_verts[feats_length[i]:feats_length[i+1], :, :3] = seq_dict[i]['pts3d'][:, None, :3]
    seq_feats_viewpoints.extend(seq_dict[i]['viewpoints'])
    seq_length.append(sum([seq_dict[i]['feats'][j].shape[0] for j in range(len(seq_dict[i]['feats']))]))
...
show_scene2d(pts2d=[seq_feats_embed[:], seq_feats_embed[:]],
             pts2d_colors=[seq_colors_padded_verts[seq_feats_padded_mask], seq_colors_padded_viewpoint[seq_feats_padded_mask]],
